...
local function creatureSayCallback(npc, creature, type, message)
	...
	elseif npcHandler:getTopic(playerId) == 2 then
		...
		if missionsTable then
			...
			if player:getStorageValue(storage[playerId]) > 0 and player:getStorageValue(storage[playerId]) == value[playerId] then
				...
				player:sendLocalizedMessage(MESSAGE_EXPERIENCE, "npc.gerimor.experience_gained", {rewardExperience[playerId]})
				npcHandler:setTopic(playerId, 0)
	...
	elseif MsgContains(message, "yes") then
		...
		elseif npcHandler:getTopic(playerId) == 4 then
			...
			player:sendLocalizedMessage(MESSAGE_EVENT_ADVANCE, "npc.gerimor.item_gained", {item})
