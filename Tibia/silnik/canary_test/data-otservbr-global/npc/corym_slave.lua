...
local function greetCallback(npc, creature, message)
	...
	if player:getStorageValue(HiddenThreats.CorymRescued02) < 0 then
		npcHandler:setLocalizedMessage(MESSAGE_GREET, "npc.corym_slave.greet_msg_2")
	else
		NPC_LIB.i18n.setLocalizedMessage(npcHandler, MESSAGE_GREET, "npc.corym_slave.greet_msg_1")
	end
	return true
end
...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "pay back") then
		NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.corym_slave.multi_1")
		if player:getStorageValue(HiddenThreats.CorymRescued02) < 0 then
...
-- Greeting message
NPC_LIB.i18n.setLocalizedMessage(npcHandler, MESSAGE_FAREWELL, "npc.corym_slave.farewell_msg_1")
