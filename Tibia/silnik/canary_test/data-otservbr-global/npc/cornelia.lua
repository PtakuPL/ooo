...
keywordHandler:addKeyword({ "job" }, StdModule.say, { npcHandler = npcHandler, i18nKey = "npc.cornelia.stdmod_1" })
...
local function creatureSayCallback(npc, creature, type, message)
	...
	if table.contains({ "addon", "armour" }, message) then
		if player:getStorageValue(Storage.Quest.U7_8.WarriorOutfits.WarriorShoulderAddon) == 5 then
			...
			NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.cornelia.say_2")
		elseif player:getStorageValue(Storage.Quest.U7_8.WarriorOutfits.WarriorShoulderAddon) == 6 then
			if player:getStorageValue(Storage.Quest.U7_8.WarriorOutfits.WarriorShoulderTimer) > os.time() then
				NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.cornelia.say_3")
			elseif player:getStorageValue(Storage.Quest.U7_8.WarriorOutfits.WarriorShoulderTimer) > 0 and player:getStorageValue(Storage.Quest.U7_8.WarriorOutfits.WarriorShoulderTimer) < os.time() then
				...
				NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.cornelia.say_4")
			else
				NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.cornelia.say_5")
			end
		end
	end
	return true
...
NPC_LIB.i18n.setLocalizedMessage(npcHandler, MESSAGE_GREET, "npc.cornelia.greet_msg_1")
NPC_LIB.i18n.setLocalizedMessage(npcHandler, MESSAGE_FAREWELL, "npc.cornelia.farewell_msg_1")
