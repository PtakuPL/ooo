...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "ticket") then
		if player:getStorageValue(Storage.WagonTicket) >= os.time() then
			npcHandler:sayLocalized("npc.gewen.your_weekly_ticket_1", npc, creature)
			return true
		end

		npcHandler:sayLocalized("npc.gewen.do_you_want_2", npc, creature)
		npcHandler:setTopic(playerId, 1)
	elseif npcHandler:getTopic(playerId) == 1 then
		if MsgContains(message, "yes") then
			if not player:removeMoneyBank(250) then
				npcHandler:sayLocalized("npc.gewen.you_dont_have_3", npc, creature)
				return true
			end

			player:setStorageValue(Storage.WagonTicket, os.time() + 7 * 24 * 60 * 60)
			npcHandler:sayLocalized("npc.gewen.here_is_your_4", npc, creature)
