...
local function greetCallback(npc, creature, message)
	...
	if player:getStorageValue(HiddenThreats.CorymRescued05) < 0 then
		npcHandler:setLocalizedMessage(MESSAGE_GREET, "npc.corym_butler.greet_msg_2")
	else
		NPC_LIB.i18n.setLocalizedMessage(npcHandler, MESSAGE_GREET, "npc.corym_butler.greet_msg_1")
	end
	return true
end
...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "repression") then
		NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.corym_butler.multi_1")
		if player:getStorageValue(HiddenThreats.CorymRescued05) < 0 then
...
-- Greeting message
NPC_LIB.i18n.setLocalizedMessage(npcHandler, MESSAGE_FAREWELL, "npc.corym_butler.farewell_msg_1")
