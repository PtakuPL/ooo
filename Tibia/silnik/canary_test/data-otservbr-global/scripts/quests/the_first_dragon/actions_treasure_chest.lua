...
function treasureChest.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if setting.name then
		player:sendLocalizedMessage(MESSAGE_EVENT_ADVANCE, "scripts.common.found_count_items", setting.count, setting.name)
		player:addItem(setting.name, setting.count, true)
	elseif setting.itemId then
		player:addItem(setting.itemId, setting.count, true)
		player:sendLocalizedMessage(MESSAGE_EVENT_ADVANCE, "scripts.common.found_count_items", setting.count, getItemName(setting.itemId))
	end
	return true
end
