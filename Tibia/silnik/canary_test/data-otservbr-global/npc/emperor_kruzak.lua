...
local function creatureSayCallback(npc, creature, type, message)
	...
	if (MsgContains(message, "outfit")) or (MsgContains(message, "addon")) then
		npcHandler:sayLocalized("npc.emperor_kruzak.in_exchange_for_1", npc, creature)
	...
	elseif MsgContains(message, "yes") then
		...
		elseif npcHandler:getTopic(playerId) == 2 then
			...
			npcHandler:sayLocalized("npc.emperor_kruzak.in_that_case_2", npc, creature)
			...
			if player:getStorageValue(Storage.Quest.U12_15.GoldenOutfits) < 1 then
				if player:getMoney() + player:getBankBalance() >= 500000000 then
					...
					if inbox and #inboxItems < inbox:getMaxCapacity() then
						...
						npcHandler:sayLocalized("npc.emperor_kruzak.take_this_armor_3", npc, creature)
						...
						npcHandler:sayLocalized("npc.emperor_kruzak.please_make_sure_4", npc, creature)
					end
				else
					npcHandler:sayLocalized("npc.emperor_kruzak.you_do_not_5", npc, creature)
				end
			else
				npcHandler:sayLocalized("npc.emperor_kruzak.you_alread_have_6", npc, creature)
			end
			npcHandler:setTopic(playerId, 2)
			-- Fim do outfit
			-- Inicio do helmet
			...
			if player:getStorageValue(Storage.Quest.U12_15.GoldenOutfits) == 1 then
				if player:getStorageValue(Storage.Quest.U12_15.GoldenOutfits) < 2 then
					if player:getMoney() + player:getBankBalance() >= 250000000 then
						npcHandler:sayLocalized("npc.emperor_kruzak.take_this_helmet_7", npc, creature)
						...
						npcHandler:sayLocalized("npc.emperor_kruzak.you_do_not_8", npc, creature)
						npcHandler:setTopic(playerId, 2)
					end
				else
					npcHandler:sayLocalized("npc.emperor_kruzak.you_alread_have_9", npc, creature)
					npcHandler:setTopic(playerId, 2)
				end
			else
				npcHandler:sayLocalized("npc.emperor_kruzak.you_need_to_10", npc, creature)
				npcHandler:setTopic(playerId, 2)
			end
			npcHandler:setTopic(playerId, 2)
			-- Fim do helmet
		...
		elseif npcHandler:getTopic(playerId) == 5 then
			if player:getStorageValue(Storage.Quest.U12_15.GoldenOutfits) == 2 then
				if player:getStorageValue(Storage.Quest.U12_15.GoldenOutfits) < 3 then
					if player:getMoney() + player:getBankBalance() >= 250000000 then
						npcHandler:sayLocalized("npc.emperor_kruzak.take_this_boots_11", npc, creature)
						player:removeMoneyBank(250000000)
						...
						npcHandler:sayLocalized("npc.emperor_kruzak.you_do_not_12", npc, creature)
						npcHandler:setTopic(playerId, 2)
					end
				else
					npcHandler:sayLocalized("npc.emperor_kruzak.you_alread_have_13", npc, creature)
					npcHandler:setTopic(playerId, 2)
				end
			else
				npcHandler:sayLocalized("npc.emperor_kruzak.you_need_to_14", npc, creature)
				npcHandler:setTopic(playerId, 2)
			end
			-- Fim da boots
			npcHandler:setTopic(playerId, 2)
	...
	elseif (MsgContains(message, "armor")) and npcHandler:getTopic(playerId) == 2 then
		npcHandler:sayLocalized("npc.emperor_kruzak.so_you_wold_15", npc, creature)
	...
	elseif (MsgContains(message, "helmet")) and npcHandler:getTopic(playerId) == 2 then
		npcHandler:sayLocalized("npc.emperor_kruzak.so_you_would_16", npc, creature)
	...
	elseif (MsgContains(message, "boots")) and npcHandler:getTopic(playerId) == 2 then
		npcHandler:sayLocalized("npc.emperor_kruzak.so_you_would_17", npc, creature)
