...
function rotwormStew.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if player:hasExhaustion("special-foods-cooldown") then
		player:sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, "scripts.rotworm_stew.msg_1")
		return true
	end

	player:addHealth(player:getMaxHealth())
	player:sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, "scripts.rotworm_stew.msg_2")
