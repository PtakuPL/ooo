...
function sweetMangonaiseElixir.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if player:hasExhaustion("special-foods-cooldown") then
		player:sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, "scripts.sweet_mangonaise_elixir.msg_1")
		return true
	end
	...
	if not playerRing or not table.contains(ringMultiplicationTable, playerRing:getId()) then
		player:sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, "scripts.sweet_mangonaise_elixir.msg_2")
	...
	if not ringId then
		player:sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, "scripts.sweet_mangonaise_elixir.msg_3")
		player:getPosition():sendMagicEffect(CONST_ME_POFF)
		return false
	end
	...
	player:sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, "scripts.sweet_mangonaise_elixir.msg_4")
