...
function coconutShrimpBake.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if player:hasExhaustion("special-foods-cooldown") then
		player:sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, "scripts.coconut_shrimp_bake.msg_1")
		return true
	end

	local headItem = player:getSlotItem(CONST_SLOT_HEAD)
	if not headItem then
		player:sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, "scripts.coconut_shrimp_bake.msg_2")
		return true
	end

	local acceptableHelmets = { 5460, 11585, 13995 }
	if not table.contains(acceptableHelmets, headItem:getId()) then
		player:sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, "scripts.coconut_shrimp_bake.msg_3")
		return true
	end

	player:sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, "scripts.coconut_shrimp_bake.msg_4" .. headItem:getName() .. " has increased for twenty-four hours.")
