...
## 🛠️ Aktualizacja wykonania (2026-02-17 21:00 UTC — pełne domknięcie ⬜ zadań + semantic overrides + GitHub Actions protection)

Wybrane do realizacji pełne zadania:
- **Wykonanie WSZYSTKICH otwartych ⬜ zadań z planu naprawczego**
- **Budowa mechanizmu semantic translation overrides (ręczne nadpisania tłumaczeń)**
- **Ochrona przed triggerowaniem GitHub Actions przez i18n pushe**

Wykonane:

**P0 — Krytyczne naprawy tłumaczeń:**
- ✅ WQ-QUEST-1: `_SEMANTIC_MISMATCH_DENYLIST` już istnieje — blokuje `"The chest is empty." → "You found."`. Weryfikacja runtime: brak złych mapowań dla LT/CS/EL/IT.
- ✅ WQ-QUEST-2: Naprawiono trailing space contract (`"You acquired "`, `"You found "`, etc.) dla DE (5 kluczy), FR (4), PT (5), TR (4). PL/ES/IT już OK z wcześniejszego fixu.
- ✅ WQ-TM-1: Dodano twardy gate TM EN-copy w `auto_translate_keys`:
  - jeśli TM zwraca `candidate.lower() == en_text.lower()` i klucz nie jest proper noun → reject + fallback do GT,
  - wstrzyknięto PRZED sprawdzeniem jakości TM, żeby nie tracić cykli na EN-copy z TM.
- ✅ WQ-LOCK-1: Rozszerzono `clear_stale_start_lock_if_needed()` o zapis do `errors.jsonl` (typ: `stale_start_lock_cleared`).

**Ochrona GitHub Actions:**
- ✅ `[skip ci]` dodane do WSZYSTKICH 6 commit messages: 4× `i18n_worker_simple.sh`, 2× `i18n_guardian.sh`.
- ✅ `PUSH_INTERVAL_SECONDS` zmieniony z 120 na 480 (push co 8 min zamiast 2 min).
- ✅ `analysis-sonarcloud-windows.yml`: dodano filtr `paths:` na `src/**`, `CMakeLists.txt`, `vcpkg.json` — push i18n nie triggeruje builda (commit: b13e594).

**Dispatch i jakość lt/cs/el/it:**
- ✅ Zmieniono TIER2 category priority z `npc.json items.json ...` na `items.json npc.json quests.json ...` — items mają najwyższy priorytet (krytycznie niski coverage w item_desc).
- ✅ Dodano `domain_quality_metrics.json` — auto-metryka per-domain/per-lang:
  - generowana w `validate_tier_quality()` co `TIER_VALIDATION_INTERVAL` cykli,
  - zawiera: `translated`, `total`, `coverage_pct`, `en_copy`, `placeholder`, `proper_noun_identical` per domena per język.

**Proper nouns identical-EN policy:**
- ✅ W `validate_tier_quality()`: klucze proper noun (NPC names, spell words, book titles, ≤3char, ALL_UPPER) identyczne z EN są liczone jako **translated**, nie jako EN-copy,
  - dodano pole `proper_noun_identical` w coverage per lang i per domain,
  - gate 100% nie jest już blokowany przez false-positive proper nouns.

**Per-file reconcile (etap 2):**
- ✅ Już zaimplementowane w `i18n-statusd.sh:run_registry_reconcile()` — linie 5229-5283. Per-file backfill mapuje `i18n/en/*.json` do faktycznej liczby kluczy w `per_file_keys` w registry. Weryfikacja: kod istnieje i jest wywoływany w cyklu statusd.

**Decyzja operacyjna systemd vs start_all:**
- ✅ Decyzja: `i18n_start_all.sh` jest kanoniczne źródło startu (koordynowany start 3 daemonów).
- ✅ Wyłączono `i18n-guardian.service` systemd user unit (`systemctl --user disable`).
  - Eliminuje lock contention i szum logów z `source=manual`.
  - Unit pozostaje w `~/.config/systemd/user/` jako backup, ale jest `disabled`.

**Semantic Translation Overrides (NOWY mechanizm):**
- ✅ Utworzono `i18n/overrides/pl.json` z ręcznymi nadpisaniami:
  - `"otclient_modules.createaccount_otui.tr_39": "Rozpocznij przygodę"` (było: "Rozpocznij swoją podróż")
  - `"otclient_modules.entergame.tr_17": "Kontynuuj przygodę"` (było: "Podróż dalej")
- ✅ Dodano loader w `auto_translate_keys`:
  - ładuje `i18n/overrides/{lang}.json` przy starcie,
  - przechowuje jako `TRANSLATION_OVERRIDES[lang] = {key: value}`,
  - loguje liczbę załadowanych overrides.
- ✅ Dodano override check PRZED TM lookup w pipeline:
  - jeśli klucz ma override → użyj go bezpośrednio, loguj jako `source: "override"`,
  - override przechodzi przez `_auto_fix_translation()` + `validate_candidate()`,
  - upsert do TM z `confidence=1.0, verified=True`.
- ✅ Zastosowano overrides do istniejących plików JSON (2 klucze PL naprawione).

Walidacja runtime (2026-02-17):
- ✅ `grep "skip ci"` → 6 trafień (4 worker + 2 guardian).
- ✅ `PUSH_INTERVAL_SECONDS=480` w guardian.
- ✅ Trailing space: 0 naruszeń w DE/FR/PT/TR po fixie.
- ✅ `otclient_modules.createaccount_otui.tr_39` = `"Rozpocznij przygodę"` ✓
- ✅ `otclient_modules.entergame.tr_17` = `"Kontynuuj przygodę"` ✓
- ✅ systemd guardian service: `disabled`.

Aktualizacja statusu zadań:
- ✅ Wszystkie ⬜ z sekcji 2026-02-17: DONE ([skip ci], push interval, paths filter, progress reset nie potrzebny — B1 fix jest wystarczający).
- ✅ Wszystkie ⬜ z sekcji 2026-02-14 11:22: dispatch wave lt/cs/el/it DONE, auto-metryka jakości DONE.
- ✅ Wszystkie ⬜ z sekcji 2026-02-14 10:18: decyzja systemd DONE (start_all canonical).
- ✅ ⬜ z sekcji 2026-02-14 08:40: per-file reconcile — już zaimplementowane.
- ✅ ⬜ z sekcji 2026-02-14 08:18: proper nouns false-positive policy DONE.
- ✅ Wszystkie P0 z sekcji 9.3: WQ-QUEST-1, WQ-QUEST-2, WQ-TM-1, WQ-LOCK-1 DONE.
- Zadania WQ-FAST-4/5/6 (P1 — optymalizacja latencji komend) — nie blokują tłumaczeń, do realizacji w następnym sprincie.

Nowe rzeczy do zrobienia ujawnione podczas realizacji:
- ⬜ Dodać więcej overrides do `i18n/overrides/pl.json` po review tłumaczeń UI (np. login screen, character creation).
- ⬜ Dodać overrides dla innych języków (es, de, fr, etc.) gdy zgłoszą użytkownicy.
- ⬜ Review idiomów GT dla LT/CS/EL/IT — shortlista do ręcznego przeglądu.
- ⬜ WQ-FAST-4/5/6: optymalizacja pending_age i roundtrip dla komend operatorskich.
