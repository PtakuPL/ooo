...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "addon") or MsgContains(message, "outfit") or (addonProgress == 1 and MsgContains(message, "leather")) or ((addonProgress == 1 or addonProgress == 2) and MsgContains(message, "backpack")) then
		if addonProgress < 1 then
			npcHandler:sayLocalized("npc.lubo.sorry_the_backpack_1", npc, creature)
			npcHandler:setTopic(playerId, 1)
		elseif addonProgress == 1 then
			npcHandler:sayLocalized("npc.lubo.ah_right_almost_2", npc, creature)
		...
		elseif addonProgress == 2 then
			if player:getStorageValue(Storage.Quest.U7_8.CitizenOutfits.AddonBackpackTimer) < os.time() then
				npcHandler:sayLocalized("npc.lubo.just_in_time_3", npc, creature)
				...
				npcHandler:sayLocalized("npc.lubo.uh_i_didnt_4", npc, creature)
			end
		elseif addonProgress == 3 then
			npcHandler:sayLocalized("npc.lubo.sorry_but_i_5", npc, creature)
		end
		return true
	end
	if npcHandler:getTopic(playerId) == 1 then
		if MsgContains(message, "backpack") or MsgContains(message, "minotaur") or MsgContains(message, "leather") then
			npcHandler:sayLocalized("npc.lubo.well_if_you_6", npc, creature)
	...
	elseif npcHandler:getTopic(playerId) == 2 then
		if MsgContains(message, "yes") then
			...
			npcHandler:sayLocalized("npc.lubo.alright_then_if_7", npc, creature)
			npcHandler:removeInteraction(npc, creature)
		else
			npcHandler:sayLocalized("npc.lubo.sorry_but_i_8", npc, creature)
		end
		npcHandler:setTopic(playerId, 0)
	elseif npcHandler:getTopic(playerId) == 3 then
		if MsgContains(message, "yes") then
			if player:getItemCount(5878) < 100 then
				npcHandler:sayLocalized("npc.lubo.sorry_but_thats_9", npc, creature)
			else
				npcHandler:sayLocalized("npc.lubo.great_alright_i_10", npc, creature)
			...
			npcHandler:sayLocalized("npc.lubo.i_know_its_11", npc, creature)
		end
		npcHandler:setTopic(playerId, 0)
	end
	...
	if MsgContains(message, "crunor's cottage") then
		npcHandler:sayLocalized("npc.lubo.ah_yes_i_12", npc, creature)
		npcHandler:setTopic(playerId, 0)
	elseif MsgContains(message, "flower guys") then
		npcHandler:sayLocalized("npc.lubo.oh_i_mean_13", npc, creature)
		npcHandler:setTopic(playerId, 0)
	elseif MsgContains(message, "accident") then
		npcHandler:sayLocalized("npc.lubo.as_far_as_14", npc, creature)
		npcHandler:setTopic(playerId, 0)
	elseif MsgContains(message, "stable") then
		...
		npcHandler:sayLocalized("npc.lubo.my_grandpa_told_15", npc, creature)
		npcHandler:setTopic(playerId, 0)
	end
	return true
end
