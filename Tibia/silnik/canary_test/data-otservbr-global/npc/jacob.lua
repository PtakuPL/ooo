...
local function creatureSayCallback(npc, creature, type, message)
	...
	elseif MsgContains(message, "mission") then
		if player:getStorageValue(Storage.Quest.U10_50.OramondQuest.TheAncientSewers.Mission) < 1 then
			NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.jacob.say_2")
			npcHandler:setTopic(playerId, 2)
		elseif player:getStorageValue(Storage.Quest.U10_50.OramondQuest.TheAncientSewers.Mission) < 22 then
			NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.jacob.say_3")
			npcHandler:setTopic(playerId, 3)
		elseif player:getStorageValue(Storage.Quest.U10_50.OramondQuest.TheAncientSewers.Mission) == 22 then
			NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.jacob.say_4")
			npcHandler:setTopic(playerId, 0)
		end
	elseif MsgContains(message, "yes") then
		if npcHandler:getTopic(playerId) == 2 then
			NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.jacob.say_5")
			player:setStorageValue(Storage.Quest.U10_50.OramondQuest.TheAncientSewers.Mission, 1)
	...
	elseif MsgContains(message, "ok") then
		if npcHandler:getTopic(playerId) == 3 then
			NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.jacob.say_6")
	...
	elseif MsgContains(message, "report") then
		if player:getStorageValue(Storage.Quest.U10_50.DarkTrails.Mission05) == 1 then
			if npcHandler:getTopic(playerId) == 7 then
				...
				NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.jacob.say_7")
			end
		end
	end
...
NPC_LIB.i18n.setLocalizedMessage(npcHandler, MESSAGE_GREET, "npc.jacob.greet_msg_1")
NPC_LIB.i18n.setLocalizedMessage(npcHandler, MESSAGE_FAREWELL, "npc.jacob.farewell_msg_1")
