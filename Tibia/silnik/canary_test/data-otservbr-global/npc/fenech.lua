...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "first rod") or MsgContains(message, "first wand") then
		if player:isMage() then
			if player:getStorageValue(Storage.FirstMageWeapon) == -1 then
				npcHandler:sayLocalized("npc.fenech.so_you_ask_1" .. ItemType(itemId):getName() .. "} to begin your adventure?", npc, creature)
				npcHandler:setTopic(playerId, 1)
			else
				npcHandler:sayLocalized("npc.fenech.what_i_have_2" .. ItemType(itemId):getName() .. "}!", npc, creature)
			end
		else
			npcHandler:sayLocalized("npc.fenech.sorry_you_arent_3", npc, creature)
		end
	elseif MsgContains(message, "yes") then
		if npcHandler:getTopic(playerId) == 1 then
			player:addItem(itemId, 1)
			npcHandler:sayLocalized("npc.fenech.here_you_are_4", npc, creature)
	...
	elseif categoryTable then
		...
		npcHandler:sayLocalized("npc.fenech.of_course_just_5" .. remainingCategories .. ".", npc, player)
