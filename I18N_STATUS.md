...
> Worker tłumaczy **53** języków. Klucze EN: **74,536**. Pokrycie globalne: **nominal 17.9% / real 10.82%**. Tempo: **756.0 kluczy/h**. Tłumaczeń netto: **138,124**.

## 🧭 META

> **[META]** 🟢 AKTYWNY  
> Świeżość: teraz | Źródło: `update_github_status()` | Ostatnia aktualizacja: 2026-04-28 23:56:59

> **Aktualizacja:** 2026-04-28 23:56:59 UTC
> **Worker:** v1.1 Simple | **Guardian:** v2.0 | **Języki:** 53 | **Klucze EN:** 74536
> **Publikacja:** git-enabled
> **LIVE:** Cykl #17,348 | Status: 🟢 DZIAŁA | Faza: AUTO_TRANSLATE | Etap: heartbeat_tick | Kategoria: es | Plik: server.json | Heartbeat: 2026-04-28T23:54:25Z
> **Okno godzinowe:** okno=1.0h | cykli=88 | pominięte=0.0% | odrzucone=4.2% | przepustowość=756.0/h  
> **Tłumaczeń netto:** 138,124

### 🧩 Status sekcji
| Sekcja | Stan | Świeżość | Powód | Źródło | Ostatnia aktualizacja |
|--------|------|----------|-------|--------|-----------------------|
| META | 🟢 AKTYWNY | teraz | - | `update_github_status()` | 2026-04-28 23:56:59 |
| LIVE | 🟢 AKTYWNY | 2min temu | - | `activity.json / worker_state.json` | 2026-04-28T23:54:25Z |
| PRE_MIGRATION | 🔒 NIEAKTYWNY | 55s temu | worker w trybie AUTO_TRANSLATE | `i18n/en/*.json (LIVE) + i18n_file_status.json + i18n_processed_files.txt` | 2026-04-28 23:56:59 |
| TRANSLATION | 🟢 AKTYWNY | 1min temu | - | `translation_guard_latest.json / translation_recent_latest.json` | 2026-04-28T23:55:59.953126Z |
| QUALITY | 🔒 NIEAKTYWNY | 2min temu | worker w trybie AUTO_TRANSLATE | `quality_audit_latest.json` | 2026-04-28T23:54:25.031216Z |
| HISTORY | 🟢 AKTYWNY | teraz | - | `daily/*.json / ops.jsonl` | 2026-04-28 23:56:59 |
| DOCTOR | 🟠 AKTYWNY | 0s temu | 1 alarmów | `doctor_alerts_latest.json` | 2026-04-28T23:57:05.442750Z |
...
> **[LIVE]** 🟢 AKTYWNY  
> Świeżość: 2min temu | Źródło: `activity.json / worker_state.json` | Ostatnia aktualizacja: 2026-04-28T23:54:25Z
...
| 🔄 **Ostatni cykl** | 37 kluczy, 1 odrzuconych, 76.0s, tryb: AUTO_TRANSLATE, cel: ru/items.json |
| ❤️ **Heartbeat** | 2026-04-28T23:54:25Z |
...
| ⏳ Oldest pending (visible tail) | 16052 s |
...
| ♻️ Deferred stats | enqueued=32, cooldown=1, deduped=1, manual_review=0, partial_staged=0, partial_completed=0 |
...
| 🌍 Recent provider mix | TM/simple(282), Free Google Translate(225), canonical_name_semantic_autofix(16), term_consistency_autofix(3) |
...
| ❤️ Pool health | `critical` (writes=526, fallbacks=10) |
...
| 🚫 Failing langs | - |
| 📈 Trend 24h | pass=437/437 (100.00%), fail=0, Δchecks=+0, dir=stable |
...
| `QUEUE_STARVATION` | ALERT | WARNING | `{'pending': 390, 'oldest_s': 16052}` | `{'pending_min': 200, 'oldest_s': 3600}` | Duży pending i stara kolejka sugerują głodzenie queue. |
...
> **ETA globalne:** ~165 dni (2,988,260 kluczy do celu 95%)
...
> Świeżość: 55s temu | Źródło: `i18n/en/*.json (LIVE) + i18n_file_status.json + i18n_processed_files.txt` | Ostatnia aktualizacja: 2026-04-28 23:56:59
...
> **[TRANSLATION]** 🟢 AKTYWNY  
> Świeżość: 1min temu | Źródło: `translation_guard_latest.json / translation_recent_latest.json` | Ostatnia aktualizacja: 2026-04-28T23:55:59.953126Z
...
- stone → каменный (item.11979.name)
- stone → каменный (item.11980.name)
- stone → каменный (item.11981.name)
- stone → каменный (item.11983.name)
- stone → каменный (item.11984.name)
- stone → каменный (item.11985.name)
- stone → каменный (item.11986.name)
- stone → каменный (item.11987.name)
- stone → каменный (item.11988.name)
- stone → каменный (item.11989.name)
- stone → каменный (item.11990.name)
- stone → каменный (item.11991.name)
- stone → каменный (item.11992.name)
- ornate tombstone → богато украшенное надгробие (item.11963.name)
- ornate tombstone → богато украшенное надгробие (item.11964.name)
- ornate tombstone → богато украшенное надгробие (item.11969.name)
- dried grass roof → крыша из сухой травы (item.1197.name)
- ornate tombstone → богато украшенное надгробие (item.11970.name)
- dried grass roof → крыша из сухой травы (item.1198.name)
- dried grass roof → крыша из сухой травы (item.1199.name)

### 🚫 Raporty strażnika jakości
- Raporty strażnika jakości: **4684**  
- Raporty blokad: **0**  
- Widoczne raporty 'nie mogę przetłumaczyć': **4537**
...
- **Pokrycie globalne (nominal):** **17.9%** (693,808/3,875,872)
- **Pokrycie globalne (real):** **10.82%** (419,211/3,875,872)
- **Kopie EN (łącznie):** **410,684**
- **Placeholdery [EN]/puste (łącznie):** **2,191,855**
...
| 🕒 Evidence window | 24h (span=29.08h) |
...
| Trend 24h `identical_to_en` + `word_salad` | ✅ PASS | window=29.08h; client.json: id=+0.00pp, ws=+0, otclient_modules.json: id=-1.38pp, ws=+0 |
...
| Metryka | Wartość |
|---------|---------|
| Okno | **1.0h** (2026-04-28T22:57:00.088657Z → 2026-04-28T23:57:00.088657Z) |
...
| Podejrzane tłumaczenia | **991** |
| Najgorsze cele (strażnik) | ro/items.json (gf=12), cs/items.json (gf=10), de/otclient_modules.json (gf=3), de/npc.json (gf=3), ru/items.json (gf=3) |
...
> Świeżość: 2min temu | Źródło: `quality_audit_latest.json` | Ostatnia aktualizacja: 2026-04-28T23:54:25.031216Z
...
|-----------|--------|-----------|
| Worker | 🟢 DZIAŁA | Cykl #17,348 |
| Heartbeat | 160s temu | 2026-04-28T23:54:25Z |
...
- **Ostatnia aktualizacja:** 2026-04-28T23:56:16.452899Z
...
- 2026-04-28 23:56:17 | AUTO_TRANSLATE:heartbeat_tick | es | ok | server.json
- 2026-04-28 23:56:00 | AUTO_TRANSLATE:auto_done | ru | ok | items.json
- 2026-04-28 23:54:44 | AUTO_TRANSLATE:heartbeat_tick | ru | ok | items.json
...
> **[HISTORY]** 🟢 AKTYWNY  
> Świeżość: teraz | Źródło: `daily/*.json / ops.jsonl` | Ostatnia aktualizacja: 2026-04-28 23:56:59

- 🤖 AUTO_TRANSLATE: AUTO_TRANSLATE_DONE [ru] → warn (files+1, translated+37, skipped+0) — lang=ru file=items.json strict_skipped_done=1792 guard_fail=1 placeholder=0 command=0 pipe=0
...
| Metryka | Wartość |
|---------|---------|
| ✅ Przetłumaczono | **8,163** kluczy |
| ⏭️ Pominięte | 0 |
| 🔁 Cykli | 127 |
| ❌ Błędów | 333 |
| 🌍 Aktywnych języków | 9 |
| 🏆 TOP 5 | **DE** (1,370), **RU** (1,269), **IT** (1,021), **TR** (916), **RO** (911) |
| 📊 Porównanie | ↑ 57.3% vs wczoraj (5,188) |

## 📆 Ten tydzień (7 dni)

> Suma tygodnia: **31,023** kluczy
...
| 🔄 Cykli wykonanych | **#17,348** | continuous mode |
...
| System | Status | Info |
|--------|--------|------|
| Worker v1.1 | 🟢 DZIAŁA | Cykl #17,348 |
...
📅 Auto-updated by Worker v1.1 | Last: 2026-04-28 23:56:59  
