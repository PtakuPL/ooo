...
bool Database::connect(const std::string* host, const std::string* user, const std::string* password, const std::string* database, uint32_t port, const std::string* sock) {
	...
	// Ensure the client connection uses utf8mb4 so all Unicode characters are preserved.
	if (mysql_set_character_set(handle, "utf8mb4") != 0) {
		g_logger().warn("Unable to set MySQL client character set to utf8mb4: {}", mysql_error(handle));
	}
