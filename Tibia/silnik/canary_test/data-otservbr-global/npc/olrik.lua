...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "measurements") then
		if player:getStorageValue(Storage.Quest.U7_24.ThePostmanMissions.Mission07) >= 1 and player:getStorageValue(Storage.Quest.U7_24.ThePostmanMissions.MeasurementsOlrik) ~= 1 then
			NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.olrik.measurements_gamble")
	...
	elseif MsgContains(message, "yes") then
		if player:getMoney() + player:getBankBalance() >= 5 then
			...
			if number ~= 6 then
				NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.olrik.dice_lose", { tostring(number) })
			else
				NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.olrik.dice_win", { tostring(number) })
