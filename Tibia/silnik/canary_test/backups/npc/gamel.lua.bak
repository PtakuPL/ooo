...
local function greetCallback(npc, creature)
	...
	if player:getStorageValue(Storage.Quest.U8_1.SecretService.AVINMission01) == 1 and player:getItemCount(402) > 0 then
		...
		NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.gamel.say_1")
		for i = 1, 2 do
			Game.createMonster("Bandit", npc:getPosition())
		end
		npcHandler:setTopic(playerId, 0)
	else
		NPC_LIB.i18n.setLocalizedMessage(npcHandler, MESSAGE_GREET, "npc.gamel.greet_msg_1")
	end
	return true
end
...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "letter") then
		if player:getStorageValue(Storage.Quest.U8_1.SecretService.AVINMission01) == 2 then
			NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.gamel.say_2")
	...
	elseif MsgContains(message, "yes") then
		if npcHandler:getTopic(playerId) == 1 then
			if player:removeItem(402, 1) then
				...
				NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.gamel.say_3")
			else
				NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.gamel.say_4")
			end
			npcHandler:setTopic(playerId, 0)
		end
	end
...
NPC_LIB.i18n.setLocalizedMessage(npcHandler, MESSAGE_WALKAWAY, "npc.gamel.walkaway_msg_1")
NPC_LIB.i18n.setLocalizedMessage(npcHandler, MESSAGE_FAREWELL, "npc.gamel.farewell_msg_1")
