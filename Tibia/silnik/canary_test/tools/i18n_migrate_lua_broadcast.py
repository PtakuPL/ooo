...
def _parse_string_format(expr: str) -> Optional[dict]:
    ...
    if fmt_literal is None:
        return None
    return {
        "text": _printf_to_braces(fmt_literal),
...
def migrate_file(
    ...
    needles = ["Game.broadcastMessage", "broadcastMessage"]
    ...
    for n in needles:
        ...
        while True:
            ...
            if idx == -1:
                break
            # Avoid double-capturing "broadcastMessage" inside "Game.broadcastMessage"
            if n == "broadcastMessage" and idx > 0 and content[idx - 1] == ".":
                start = idx + len(n)
                continue
            if content.startswith("Game.broadcastLocalizedMessage", idx) or content.startswith(
                "broadcastLocalizedMessage", idx
            ):
                start = idx + len(n)
                continue
    ...
    for idx, needle in positions:
        ...
        message_type = args[1].strip() if len(args) >= 2 else "MESSAGE_GAME_HIGHLIGHT"
        new_args: List[str] = [f"\"{key}\"", message_type]
        if fmt_args:
            new_args.append("{" + ", ".join(fmt_args) + "}")

        new_call = "Game.broadcastLocalizedMessage(" + ", ".join(new_args) + ")"
...
def main() -> int:
    ...
    ap.add_argument("--suffix", default="broadcast")
    args = ap.parse_args()
