...
function elementalSpheresMachine2.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if table.contains({ 844, 845 }, item.itemid) then
		...
		player:sayLocalized("scripts.actions_machine2.say_2", TALKTYPE_MONSTER_SAY, false, player, toPosition)
	else
		...
		item:transform(item.itemid - 4)
		player:sayLocalized("scripts.actions_machine2.say_1", TALKTYPE_MONSTER_SAY, false, player, toPosition)
	end
	return true
end
