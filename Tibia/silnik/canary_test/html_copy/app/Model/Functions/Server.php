...
{
    // 1) Pobierz IP/port świata z bazy (poprawne użycie: order jako 2. parametr, limit jako 3.)
    try {
        $world = (new Database('canary_worlds'))
            ->select(null, 'id ASC', '1', ['ip', 'port'])
            ->fetchObject();
    } catch (\Throwable $e) {
        return 'Server Offline';
    }

    if (!$world || empty($world->ip) || empty($world->port)) {
        return 'Server Offline';
    }
    ...
    // 2) Szybki test gniazda TCP (status protocol)
    $errno = 0;
    $errstr = '';
    $online = false;

    $sock = @stream_socket_client("tcp://{$ip}:{$port}", $errno, $errstr, 1.0);
    if (is_resource($sock)) {
        $online = true;
        fclose($sock);
    }
    ...
    // 3) Serwer żyje – sprawdź ilu graczy jest online (jeśli tabela pusta, pokaż online)
    try {
        $row = (new Database('players_online'))
            ->select(null, null, null, ['COUNT(*) AS qtd'])
            ->fetchObject();
        $qtd = (int)($row->qtd ?? 0);
    } catch (\Throwable $e) {
        $qtd = 0;
    ...
    return $qtd > 0 ? 'Server Online' : 'Server Online (no players)';
}
