...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "mission") then
		...
		elseif player:getStorageValue(Storage.Quest.U8_1.TowerDefenceQuest.Questline) == 4 then
			npcHandler:sayLocalized("npc.bunny_bonecrusher.alright_you_delivered_1", npc, creature)
			npcHandler:setTopic(playerId, 2)
		end
	elseif npcHandler:getTopic(playerId) == 1 and MsgContains(message, "yes") then
		npcHandler:sayLocalized("npc.bunny_bonecrusher.i_am_not_2", npc, creature)
	...
	elseif npcHandler:getTopic(playerId) == 2 and MsgContains(message, "password*") then
		npcHandler:sayLocalized("npc.bunny_bonecrusher.thats_right_here_3", npc, creature)
