		...
		if chance >= randomItem.from and chance <= randomItem.to then
			if randomItem.itemId then
				...
				if item.itemid == 12413 then
					...
					player:sayLocalized("scripts.random_items.found_in_bag", TALKTYPE_MONSTER_SAY, false, nil, nil, { (count > 1 and count or (itemType:getArticle() ~= "" and itemType:getArticle() or "")) .. " " .. (count > 1 and itemType:getPluralName() or itemType:getName()) })
				end
			else
