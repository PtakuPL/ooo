...
function spell.onCastSpell(creature, variant)
	...
	local stringLevelKey = "scripts.find_fiend.level_unknown"
	if monsterType and creature:isMonsterBestiaryUnlocked(monsterType:raceId()) then
		...
		if bestiaryKillsAmount >= 5 and bestiaryKillsAmount <= 25 then
			stringLevelKey = "scripts.find_fiend.level_harmless"
		elseif bestiaryKillsAmount <= 250 then
			stringLevelKey = "scripts.find_fiend.level_trivial"
		elseif bestiaryKillsAmount <= 500 then
			stringLevelKey = "scripts.find_fiend.level_easy"
		elseif bestiaryKillsAmount <= 1000 then
			stringLevelKey = "scripts.find_fiend.level_medium"
		elseif bestiaryKillsAmount <= 2500 then
			stringLevelKey = "scripts.find_fiend.level_hard"
		elseif bestiaryKillsAmount <= 5000 then
			stringLevelKey = "scripts.find_fiend.level_challenging"
		end
	end

	local stringLevel = Translator.getTranslation(player, stringLevelKey)
	local monsterMsgTemplate = Translator.getTranslation(player, "scripts.find_fiend.monster_msg")
	local message = string.format(monsterMsgTemplate, msgText, stringLevel)
