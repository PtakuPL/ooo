...
function heroRathletonProfessor.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if Game.getStorageValue(GlobalStorage.HeroRathleton.MaxxenRunning) >= 1 then
		player:sayLocalized("scripts.actions_machines_professor_maxxen.say_3", TALKTYPE_MONSTER_SAY, false, nil, toPosition)
		return true
	end

	if Game.getStorageValue(GlobalStorage.HeroRathleton.ThirdMachines) == 7 then
		player:sayLocalized("scripts.actions_machines_professor_maxxen.say_2", TALKTYPE_MONSTER_SAY)
	end

	item:transform(21744)
	addEvent(revertMachine, 10 * 60 * 1000, toPosition, 21744, 21743)
	Game.setStorageValue(GlobalStorage.HeroRathleton.ThirdMachines, Game.getStorageValue(GlobalStorage.HeroRathleton.ThirdMachines) + 1)
	player:sayLocalized("scripts.actions_machines_professor_maxxen.say_1", TALKTYPE_MONSTER_SAY, false, nil, toPosition)
	return true
end
