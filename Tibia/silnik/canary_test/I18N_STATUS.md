...
> Worker tłumaczy **53** języków. Klucze EN: **74,536**. Pokrycie globalne: **nominal 18.02% / real 10.94%**. Tempo: **730.0 kluczy/h**. Tłumaczeń netto: **146,542**.

## 🧭 META

> **[META]** 🟢 AKTYWNY  
> Świeżość: teraz | Źródło: `update_github_status()` | Ostatnia aktualizacja: 2026-04-29 10:46:12

> **Aktualizacja:** 2026-04-29 10:46:12 UTC
> **Worker:** v1.1 Simple | **Guardian:** v2.0 | **Języki:** 53 | **Klucze EN:** 74536
> **Publikacja:** git-enabled
> **LIVE:** Cykl #18,244 | Status: 🟢 DZIAŁA | Faza: AUTO_TRANSLATE | Etap: heartbeat_tick | Kategoria: it | Plik: items.json | Heartbeat: 2026-04-29T10:43:32Z
> **Okno godzinowe:** okno=1.0h | cykli=84 | pominięte=0.0% | odrzucone=6.0% | przepustowość=730.0/h  
> **Tłumaczeń netto:** 146,542

### 🧩 Status sekcji
| Sekcja | Stan | Świeżość | Powód | Źródło | Ostatnia aktualizacja |
|--------|------|----------|-------|--------|-----------------------|
| META | 🟢 AKTYWNY | teraz | - | `update_github_status()` | 2026-04-29 10:46:12 |
| LIVE | 🟢 AKTYWNY | 2min temu | - | `activity.json / worker_state.json` | 2026-04-29T10:43:32Z |
| PRE_MIGRATION | 🔒 NIEAKTYWNY | 36s temu | worker w trybie AUTO_TRANSLATE | `i18n/en/*.json (LIVE) + i18n_file_status.json + i18n_processed_files.txt` | 2026-04-29 10:46:12 |
| TRANSLATION | 🟢 AKTYWNY | 1min temu | - | `translation_guard_latest.json / translation_recent_latest.json` | 2026-04-29T10:45:07.071034Z |
| QUALITY | 🔒 NIEAKTYWNY | 3min temu | worker w trybie AUTO_TRANSLATE | `quality_audit_latest.json` | 2026-04-29T10:43:13.186649Z |
| HISTORY | 🟢 AKTYWNY | teraz | - | `daily/*.json / ops.jsonl` | 2026-04-29 10:46:12 |
| DOCTOR | 🟠 AKTYWNY | 0s temu | 1 alarmów | `doctor_alerts_latest.json` | 2026-04-29T10:46:19.392422Z |
...
> **[LIVE]** 🟢 AKTYWNY  
> Świeżość: 2min temu | Źródło: `activity.json / worker_state.json` | Ostatnia aktualizacja: 2026-04-29T10:43:32Z
...
| 🔄 **Ostatni cykl** | 34 kluczy, 1 odrzuconych, 96.0s, tryb: AUTO_TRANSLATE, cel: it/items.json |
| ❤️ **Heartbeat** | 2026-04-29T10:43:32Z |
...
| ⏳ Oldest pending (visible tail) | 15488 s |
...
| 🌍 Recent provider mix | TM/simple(333), Free Google Translate(141), canonical_name_semantic_autofix(17), term_consistency_autofix(3) |
...
| ❤️ Pool health | `critical` (writes=494, fallbacks=24) |
...
| 🚫 Failing langs | - |
| 📈 Trend 24h | pass=934/934 (100.00%), fail=0, Δchecks=+0, dir=stable |
...
| `QUEUE_STARVATION` | ALERT | WARNING | `{'pending': 385, 'oldest_s': 15488}` | `{'pending_min': 200, 'oldest_s': 3600}` | Duży pending i stara kolejka sugerują głodzenie queue. |
...
> Świeżość: 36s temu | Źródło: `i18n/en/*.json (LIVE) + i18n_file_status.json + i18n_processed_files.txt` | Ostatnia aktualizacja: 2026-04-29 10:46:12
...
> **[TRANSLATION]** 🟢 AKTYWNY  
> Świeżość: 1min temu | Źródło: `translation_guard_latest.json / translation_recent_latest.json` | Ostatnia aktualizacja: 2026-04-29T10:45:07.071034Z
...
- lampions → lampioni (item.13911.name)
- lampions → lampioni (item.13912.name)
- ornamented wall → muro ornato (item.1392.name)
- RESERVED SPRITE → SPRITE RISERVATO (item.13925.name)
- RESERVED SPRITE → SPRITE RISERVATO (item.13926.name)
- RESERVED SPRITE → SPRITE RISERVATO (item.13927.name)
- RESERVED SPRITE → SPRITE RISERVATO (item.13928.name)
- RESERVED SPRITE → SPRITE RISERVATO (item.13929.name)
- ornamented wall → muro ornato (item.1393.name)
- RESERVED SPRITE → SPRITE RISERVATO (item.13930.name)
- RESERVED SPRITE → SPRITE RISERVATO (item.13931.name)
- RESERVED SPRITE → SPRITE RISERVATO (item.13932.name)
- RESERVED SPRITE → SPRITE RISERVATO (item.13933.name)
- ornamented wall → muro ornato (item.1394.name)
- hivecomb floor → pavimento dell'alveare (item.13943.name)
- hivecomb floor → pavimento dell'alveare (item.13944.name)
- drinks → bevande (item.13908.name)
- drinks → bevande (item.13909.name)
- golden anchor → ancora d'oro (item.13941.name)
- golden anchor → ancora d'oro (item.13942.name)

### 🚫 Raporty strażnika jakości
- Raporty strażnika jakości: **4909**  
- Raporty blokad: **0**  
- Widoczne raporty 'nie mogę przetłumaczyć': **4762**

### 🧱 Blockers snapshot (strict)
| State | Candidates | Missing files | Missing keys | Reason | Timestamp |
|-------|------------|---------------|--------------|--------|-----------|
| 🟠 active | 96 | 0 | 0 | - | 2026-04-29T10:43:31.071157Z |

### 🌐 Globalne info wszystkich języków
- **Pokrycie globalne (nominal):** **18.02%** (698,544/3,875,872)
- **Pokrycie globalne (real):** **10.94%** (424,211/3,875,872)
- **Kopie EN (łącznie):** **410,420**
- **Placeholdery [EN]/puste (łącznie):** **2,187,119**
...
| 🕒 Evidence window | 24h (span=39.90h) |
...
| Trend 24h `identical_to_en` + `word_salad` | ✅ PASS | window=39.90h; client.json: id=+0.00pp, ws=+0, otclient_modules.json: id=-1.38pp, ws=+0 |
...
| Metryka | Wartość |
|---------|---------|
| Okno | **1.0h** (2026-04-29T09:46:13.258259Z → 2026-04-29T10:46:13.258259Z) |
...
| Przepustowość (kluczy/h) | **730.0 kluczy/h** |
| Podejrzane tłumaczenia | **1118** |
...
> Świeżość: 3min temu | Źródło: `quality_audit_latest.json` | Ostatnia aktualizacja: 2026-04-29T10:43:13.186649Z

- **Ostatni audyt:** OK | 0 issue(s) / 100 entries | 2026-04-29T10:43:13.186649Z
- **Top 5 typów problemów:** suspicious_rejected_critical=189, suspicious_log_high=117, suspicious_log_low=82, suspicious_rejected_high=11, suspicious_log_medium=1
- **Języki o najsłabszej jakości:** es(60.1, issues=744499), ro(60.1, issues=440180), de(60.1, issues=122966), cs(60.1, issues=109487), fr(60.1, issues=60621)
- **Pliki:** `i18n/status/quality_audit_latest.json`, `i18n/status/quality_dashboard.json`, `i18n/status/quality_report.jsonl`

### 📈 Statystyki Pracy
| Metryka | Wartość | Info |
|---------|---------|------|
| 🔄 Cykl aktualny | **#163** | od uruchomienia |
...
⚠️ **ES**: jakość 60%, 744499 problemów
⚠️ **PT**: jakość 91%, 57675 problemów
⚠️ **FR**: jakość 60%, 60621 problemów
⚠️ **IT**: jakość 78%, 176944 problemów
⚠️ **RU**: jakość 64%, 370430 problemów
...
|-----------|--------|-----------|
| Worker | 🟢 DZIAŁA | Cykl #18,244 |
| Heartbeat | 167s temu | 2026-04-29T10:43:32Z |
...
- **Ostatnia aktualizacja:** 2026-04-29T10:45:21.528873Z
...
- 2026-04-29 10:45:22 | AUTO_TRANSLATE:heartbeat_tick | es | ok | server.json
- 2026-04-29 10:45:07 | AUTO_TRANSLATE:auto_done | it | ok | items.json
- 2026-04-29 10:45:02 | AUTO_TRANSLATE:heartbeat_tick | it | ok | items.json
- 2026-04-29 10:43:32 | AUTO_TRANSLATE:heartbeat_tick | it | ok | items.json
- 2026-04-29 10:43:31 | AUTO_TRANSLATE:auto_start | it | ok | items.json
- 2026-04-29 10:43:12 | AUTO_TRANSLATE:auto_done | ro | ok | items.json
...
|-----|---------|--------|--------|
| Tłumaczeń netto | **146,542** | — | 📊 |
| Adaptacyjna paczka | batch=50, gf_rate=7.47%, reason=stable_fail_rate=7.5%, filter=exclude_repair | odrzucenia <5% → zwiększ | 📊 |
...
> **[HISTORY]** 🟢 AKTYWNY  
> Świeżość: teraz | Źródło: `daily/*.json / ops.jsonl` | Ostatnia aktualizacja: 2026-04-29 10:46:12

- 🤖 AUTO_TRANSLATE: AUTO_TRANSLATE_DONE [it] → warn (files+1, translated+34, skipped+0) — lang=it file=items.json strict_skipped_done=3580 guard_fail=1 placeholder=0 command=0 pipe=0
...
- 🤖 AUTO_TRANSLATE: AUTO_TRANSLATE_DONE [es] → warn (files+1, translated+18, skipped+0) — lang=es file=items.json strict_skipped_done=1102
- 🤖 AUTO_TRANSLATE: AUTO_TRANSLATE_DONE [nl] → warn (files+1, translated+33, skipped+0) — lang=nl file=server.json strict_skipped_done=101 guard_fail=15 placeholder=9 command=0 pipe=0
- • DOCTOR_REMEDIATION: AUTO_COMMAND [-] → ok — action=force_auto reason=deferred_or_dlq_pressure
- 🤖 AUTO_TRANSLATE: AUTO_TRANSLATE_DONE [pt] → warn (files+1, translated+171, skipped+0) — lang=pt file=npc.json strict_skipped_done=3322 guard_fail=73 placeholder=51 command=0 pipe=0
- 🤖 AUTO_TRANSLATE: REPAIR_IDENTICAL_DONE [es] → ok (translated+0) — repair_identical lang=es file=otclient_modules.json target_identical=37 limit=180 tier=low_backlog domain_cap=220 gt=true suspicious_pct=5.76
...
| Metryka | Wartość |
|---------|---------|
| ✅ Przetłumaczono | **9,086** kluczy |
| ⏭️ Pominięte | 0 |
| 🔁 Cykli | 185 |
| ❌ Błędów | 392 |
| 🌍 Aktywnych języków | 7 |
| 🏆 TOP 5 | **CS** (1,629), **RU** (1,568), **IT** (1,374), **TR** (1,276), **DE** (1,260) |
| 📊 Porównanie | ↑ 10.4% vs wczoraj (8,228) |

## 📆 Ten tydzień (7 dni)

> Suma tygodnia: **28,066** kluczy
...
| 🔄 Cykli wykonanych | **#18,244** | continuous mode |
...
| System | Status | Info |
|--------|--------|------|
| Worker v1.1 | 🟢 DZIAŁA | Cykl #18,244 |
...
📅 Auto-updated by Worker v1.1 | Last: 2026-04-29 10:46:12  
