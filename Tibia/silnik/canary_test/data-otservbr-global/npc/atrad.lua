...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "addon") or MsgContains(message, "outfit") then
		if getPlayerStorageValue(creature, Storage.Atrad) < 1 then
			NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.atrad.say_1")
	...
	elseif MsgContains(message, "nose ring") or MsgContains(message, "ring") then
		if getPlayerStorageValue(creature, Storage.Atrad) == 1 then
			if (getPlayerItemCount(creature, 5804) >= 1) and getPlayerItemCount(creature, 5930) >= 1 then
				NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.atrad.say_2")
				...
				NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.atrad.say_3")
				npcHandler:setTopic(playerId, 0)
			end
		end
	elseif MsgContains(message, "yes") then
		if npcHandler:getTopic(playerId) == 2 then
			NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.atrad.say_4")
			npcHandler:setTopic(playerId, 3)
		elseif npcHandler:getTopic(playerId) == 3 then
			NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.atrad.say_5")
