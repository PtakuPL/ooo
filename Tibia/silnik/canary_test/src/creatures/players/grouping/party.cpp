...
void Party::disband() {
	...
	currentLeader->sendLocalizedTextMessage(MESSAGE_PARTY_MANAGEMENT, "server.party.msg_1");
	...
	for (const auto &member : members) {
		...
		member->sendLocalizedTextMessage(MESSAGE_PARTY_MANAGEMENT, "server.party.msg_2");
	}
...
bool Party::leaveParty(const std::shared_ptr<Player> &player, bool forceRemove /* = false */) {
	...
	player->sendLocalizedTextMessage(MESSAGE_PARTY_MANAGEMENT, "server.party.msg_3");
...
bool Party::passPartyLeadership(const std::shared_ptr<Player> &player) {
	...
	player->sendLocalizedTextMessage(MESSAGE_PARTY_MANAGEMENT, "server.party.msg_4");
	return true;
}
...
bool Party::setSharedExperience(const std::shared_ptr<Player> &player, bool newSharedExpActive, bool silent /*= false*/) {
	...
	if (newSharedExpActive) {
		...
		if (!silent) {
			leader->sendLocalizedTextMessage(MESSAGE_PARTY_MANAGEMENT, "server.party.msg_5");
		}
	}
