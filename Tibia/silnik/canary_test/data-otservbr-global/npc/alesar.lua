...
local function greetCallback(npc, creature, message)
	...
	if player:getStorageValue(Storage.Quest.U7_4.DjinnWar.EfreetFaction.Mission03) ~= 3 then
		if not MsgContains(message, "djanni'hah") then
			npcHandler:sayLocalized("npc.alesar.shove_off_little_1", npc, creature)
	...
	npcHandler:sayLocalized("npc.alesar.what_do_you_2", npc, creature)
	...
	if MsgContains(message, "mission") then
		if player:getStorageValue(Storage.Quest.U7_4.DjinnWar.EfreetFaction.Mission01) == 3 then
			...
			elseif isInArray({ 1, 2 }, missionProgress) then
				npcHandler:sayLocalized("npc.alesar.did_you_find_3", npc, creature)
				npcHandler:setTopic(playerId, 2)
			else
				npcHandler:sayLocalized("npc.alesar.dont_forget_to_4", npc, creature)
			end
		end
	...
	elseif npcHandler:getTopic(playerId) == 2 then
		if MsgContains(message, "yes") then
			if player:getItemCount(3233) == 0 or missionProgress ~= 2 then
				npcHandler:sayLocalized("npc.alesar.as_i_expected_5", npc, creature)
				npcHandler:setTopic(playerId, 1)
			else
				npcHandler:say({
		...
		elseif MsgContains(message, "no") then
			npcHandler:sayLocalized("npc.alesar.as_i_expected_6", npc, creature)
			npcHandler:setTopic(playerId, 1)
		end
	end
...
local function onTradeRequest(npc, creature)
	...
	if player:getStorageValue(Storage.Quest.U7_4.DjinnWar.EfreetFaction.Mission03) ~= 3 then
		npcHandler:sayLocalized("npc.alesar.im_sorry_but_7", npc, creature)
		return false
	end
