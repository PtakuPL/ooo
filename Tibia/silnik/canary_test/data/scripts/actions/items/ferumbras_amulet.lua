...
function ferumbrasAscendantAmulet.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if amulet ~= item then
		player:sendLocalizedMessage(MESSAGE_EVENT_ADVANCE, "scripts.ferumbras_amulet.msg_1")
		return true
	end
	...
	if item.itemid == 22767 then
		if math.random(2) == 1 then
			...
			player:sendLocalizedMessage(MESSAGE_EVENT_ADVANCE, "scripts.ferumbras_amulet.msg_2")
		else
			player:addMana(1000, true, true)
			player:sendLocalizedMessage(MESSAGE_EVENT_ADVANCE, "scripts.ferumbras_amulet.msg_3")
		end
	...
	elseif item.itemid == 22768 then
		player:sendLocalizedMessage(MESSAGE_EVENT_ADVANCE, "scripts.ferumbras_amulet.msg_4")
	end
	return true
end
