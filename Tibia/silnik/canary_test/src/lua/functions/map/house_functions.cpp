...
int HouseFunctions::luaHouseSetNewOwnerGuid(lua_State* L) {
	...
	if (house) {
		...
		if (isTransferOnRestart && house->hasNewOwnership()) {
			...
			if (player) {
				player->sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, "server.house_functions.msg_1");
			}
			lua_pushnil(L);
			return 1;
		}
...
int HouseFunctions::luaHouseStartTrade(lua_State* L) {
	...
	auto isTransferOnRestart = g_configManager().getBoolean(TOGGLE_HOUSE_TRANSFER_ON_SERVER_RESTART);
	if (isTransferOnRestart && house->hasNewOwnership()) {
		tradePartner->sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, "server.house_functions.msg_2");
		player->sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, "server.house_functions.msg_3");
		lua_pushnumber(L, RETURNVALUE_YOUCANNOTTRADETHISHOUSE);
		return 1;
	}
