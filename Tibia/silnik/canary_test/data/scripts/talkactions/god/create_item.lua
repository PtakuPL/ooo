...
function createItem.onSay(player, words, param)
	...
	if itemType:getId() == 0 then
		...
		if not tonumber(split[1]) or itemType:getId() == 0 then
			player:sendLocalizedTextMessage(MESSAGE_FAILURE, "talkaction.god.create_item.msg_no_item")
			return true
		end
	end
	...
	if not tier then
		...
		if tier <= 0 or tier > 10 then
			player:sendLocalizedTextMessage(MESSAGE_FAILURE, "talkaction.god.create_item.msg_invalid_tier")
			return true
		else
