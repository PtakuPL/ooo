	...
	if item.itemid == 2772 then
		...
		for i = 1, #setting.playerPositions do
			...
			if not creature or not creature:isPlayer() then
				player:sendLocalizedCancelMessage("quests.dark_trails.you_need_5_players_to_fight")
				return true
			end
		...
		if #storePlayers < 5 then
			player:sendLocalizedCancelMessage("quests.dark_trails.you_need_exactly_5_players_to")
			return true
		end
