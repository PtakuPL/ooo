...
- [ ] Tabele tworzone idempotentnie (IF NOT EXISTS)
- [ ] Rollback każdej migracji odwraca rollout
- [ ] `_migrations` tabela śledzi stan
- [ ] Cleanup events działają (test: `SHOW EVENTS`)
- [ ] migrate.php działa z CLI
...
| `status.checked` | server-status.php | servers_online_count |
