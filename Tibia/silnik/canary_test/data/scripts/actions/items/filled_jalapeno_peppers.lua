...
function filledJalapenoPeppers.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if player:hasExhaustion("special-foods-cooldown") then
		player:sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, "scripts.filled_jalapeno_peppers.msg_1")
		return true
	end

	player:addCondition(speedCondition)
	player:sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, "scripts.filled_jalapeno_peppers.msg_2")
