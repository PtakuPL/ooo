...
function cultsOfTibiaLevers.onUse(player, item, fromPosition, itemEx, toPosition)
	...
	if item:getActionId() == 5501 and item:getId() == 8912 then
		if player:getPosition() == Position(33138, 31953, 15) then
			...
			if teleport ~= 5 then
				player:sendLocalizedCancelMessage("quests.cults_of_tibia.you_need_exactly_5_players_to")
				return true
			end

			if isPlayerInArea(frompos, topos) then
				player:sendLocalizedCancelMessage("quests.cults_of_tibia.the_room_is_full")
				return true
			end
	...
	if item:getActionId() == 5500 then
		if player:getPosition() == Position(33095, 31943, 15) and item:getId() == 8912 then
			...
			if isPlayerInArea(frompos, topos) then
				player:sendLocalizedCancelMessage("quests.cults_of_tibia.it_looks_like_there_is_someone")
				return true
			end
	...
	if item:getActionId() == 5506 then
		if player:getPosition() == Position(33074, 31884, 15) and item:getId() == 8912 then
			...
			if teleport ~= 10 then
				player:sendLocalizedCancelMessage("quests.cults_of_tibia.you_need_exactly_10_players_to")
				return true
			end
