...
function blueberryCupcake.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if player:hasExhaustion("blueberry-cupcake-cooldown") then
		player:sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, "scripts.blueberry_cupcake.msg_1")
		return true
	end

	player:addMana(player:getMaxMana())
	player:sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, "scripts.blueberry_cupcake.msg_2")
