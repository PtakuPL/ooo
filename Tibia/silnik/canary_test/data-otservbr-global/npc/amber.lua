...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "addon") or MsgContains(message, "outfit") or (addonProgress == 1 and MsgContains(message, "leather")) or ((addonProgress == 1 or addonProgress == 2) and MsgContains(message, "backpack")) then
		if addonProgress < 1 then
			npcHandler:sayLocalized("npc.amber.ah_you_noticed_1", npc, creature)
			npcHandler:setTopic(playerId, 1)
		elseif addonProgress == 1 then
			npcHandler:sayLocalized("npc.amber.ah_right_almost_2", npc, creature)
		...
		elseif addonProgress == 2 then
			if player:getStorageValue(Storage.Quest.U7_8.CitizenOutfitsRook.AddonBackpackRookTimer) < os.time() then
				npcHandler:sayLocalized("npc.amber.just_in_time_3", npc, creature)
				...
				npcHandler:sayLocalized("npc.amber.uh_i_didnt_4", npc, creature)
			end
		elseif addonProgress == 3 then
			npcHandler:sayLocalized("npc.amber.sorry_but_i_5", npc, creature)
		end
		return true
	end
	...
	if npcHandler:getTopic(playerId) == 1 then
		if MsgContains(message, "backpack") or MsgContains(message, "minotaur") or MsgContains(message, "leather") then
			npcHandler:sayLocalized("npc.amber.well_if_you_6", npc, creature)
	...
	elseif npcHandler:getTopic(playerId) == 2 then
		if MsgContains(message, "yes") then
			...
			npcHandler:sayLocalized("npc.amber.alright_then_if_7", npc, creature)
			npcHandler:removeInteraction(npc, creature)
		else
			npcHandler:sayLocalized("npc.amber.sorry_but_i_8", npc, creature)
		end
		npcHandler:setTopic(playerId, 0)
	elseif npcHandler:getTopic(playerId) == 3 then
		if MsgContains(message, "yes") then
			if player:getItemCount(5878) < 100 then
				npcHandler:sayLocalized("npc.amber.sorry_but_thats_9", npc, creature)
			else
				npcHandler:sayLocalized("npc.amber.great_alright_i_10", npc, creature)
			...
			npcHandler:sayLocalized("npc.amber.i_know_its_11", npc, creature)
		end
		npcHandler:setTopic(playerId, 0)
	end
end
