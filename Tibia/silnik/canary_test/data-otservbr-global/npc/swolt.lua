...
local function creatureSayCallback(npc, creature, type, message)
	...
	elseif MsgContains(message, "yes") and npcHandler:getTopic(playerId) == 1 then
		if player:getItemCount(10328) > 0 then
			NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.swolt.say_3", { player:getItemCount(10328) * 10 })
...
-- Basic
keywordHandler:addKeyword({ "job" }, StdModule.say, { npcHandler = npcHandler, i18nKey = "npc.swolt.stdmod_1" })
keywordHandler:addKeyword({ "food" }, StdModule.say, { npcHandler = npcHandler, i18nKey = "npc.swolt.stdmod_2" })
