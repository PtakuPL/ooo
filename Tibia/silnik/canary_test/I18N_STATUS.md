...
> Worker tłumaczy **53** języków. Klucze EN: **74,536**. Pokrycie globalne: **nominal 18.05% / real 10.97%**. Tempo: **602.0 kluczy/h**. Tłumaczeń netto: **148,340**.

## 🧭 META

> **[META]** 🟢 AKTYWNY  
> Świeżość: teraz | Źródło: `update_github_status()` | Ostatnia aktualizacja: 2026-04-29 13:29:05

> **Aktualizacja:** 2026-04-29 13:29:05 UTC
> **Worker:** v1.1 Simple | **Guardian:** v2.0 | **Języki:** 53 | **Klucze EN:** 74536
> **Publikacja:** git-enabled
> **LIVE:** Cykl #18,464 | Status: 🟢 DZIAŁA | Faza: AUTO_TRANSLATE | Etap: heartbeat_tick | Kategoria: es | Plik: items.json | Heartbeat: 2026-04-29T13:25:27Z
> **Okno godzinowe:** okno=1.0h | cykli=76 | pominięte=0.0% | odrzucone=6.1% | przepustowość=602.0/h  
> **Tłumaczeń netto:** 148,340

### 🧩 Status sekcji
| Sekcja | Stan | Świeżość | Powód | Źródło | Ostatnia aktualizacja |
|--------|------|----------|-------|--------|-----------------------|
| META | 🟢 AKTYWNY | teraz | - | `update_github_status()` | 2026-04-29 13:29:05 |
| LIVE | 🟢 AKTYWNY | 3min temu | - | `activity.json / worker_state.json` | 2026-04-29T13:25:27Z |
| PRE_MIGRATION | 🔒 NIEAKTYWNY | 1min temu | worker w trybie AUTO_TRANSLATE | `i18n/en/*.json (LIVE) + i18n_file_status.json + i18n_processed_files.txt` | 2026-04-29 13:29:05 |
| TRANSLATION | 🟢 AKTYWNY | 4min temu | - | `translation_guard_latest.json / translation_recent_latest.json` | 2026-04-29T13:24:36.522562Z |
| QUALITY | 🔒 NIEAKTYWNY | 4min temu | worker w trybie AUTO_TRANSLATE | `quality_audit_latest.json` | 2026-04-29T13:24:38.652087Z |
| HISTORY | 🟢 AKTYWNY | teraz | - | `daily/*.json / ops.jsonl` | 2026-04-29 13:29:05 |
| DOCTOR | 🟠 AKTYWNY | 0s temu | 1 alarmów | `doctor_alerts_latest.json` | 2026-04-29T13:29:14.453715Z |
...
> **[LIVE]** 🟢 AKTYWNY  
> Świeżość: 3min temu | Źródło: `activity.json / worker_state.json` | Ostatnia aktualizacja: 2026-04-29T13:25:27Z
...
| 🔄 **Ostatni cykl** | 39 kluczy, 4 odrzuconych, 173.3s, tryb: AUTO_TRANSLATE, cel: ro/items.json |
| ❤️ **Heartbeat** | 2026-04-29T13:25:27Z |
...
| ⏳ Oldest pending (visible tail) | 17236 s |
...
| ♻️ Deferred stats | enqueued=0, cooldown=1, deduped=1, manual_review=31, partial_staged=0, partial_completed=0 |
...
| 🌍 Recent provider mix | TM/simple(336), Free Google Translate(152), canonical_name_semantic_autofix(17), term_consistency_autofix(3) |
...
| ❤️ Pool health | `critical` (writes=508, fallbacks=23) |
...
| `QUEUE_STARVATION` | ALERT | WARNING | `{'pending': 397, 'oldest_s': 17236}` | `{'pending_min': 200, 'oldest_s': 3600}` | Duży pending i stara kolejka sugerują głodzenie queue. |
...
> **ETA globalne:** ~206 dni (2,982,588 kluczy do celu 95%)

| Język | Pasek | Pokrycie | Przetłumaczono | ETA do 95% |
|-------|-------|----------|---------------|------------|
| ES | ███████████████░░░░░ | 79.2% | 59,025/74,536 | ~20h |
...
> Świeżość: 1min temu | Źródło: `i18n/en/*.json (LIVE) + i18n_file_status.json + i18n_processed_files.txt` | Ostatnia aktualizacja: 2026-04-29 13:29:05
...
> **[TRANSLATION]** 🟢 AKTYWNY  
> Świeżość: 4min temu | Źródło: `translation_guard_latest.json / translation_recent_latest.json` | Ostatnia aktualizacja: 2026-04-29T13:24:36.522562Z
...
- RESERVED SPRITE → SPRITE RISERVATO (item.14307.name)
- RESERVED SPRITE → SPRITE RISERVATO (item.14308.name)
- RESERVED SPRITE → SPRITE RISERVATO (item.14309.name)
- ornamented wall → muro ornato (item.1431.name)
- RESERVED SPRITE → SPRITE RISERVATO (item.14310.name)
- RESERVED SPRITE → SPRITE RISERVATO (item.14311.name)
- RESERVED SPRITE → SPRITE RISERVATO (item.14312.name)
- RESERVED SPRITE → SPRITE RISERVATO (item.14313.name)
- RESERVED SPRITE → SPRITE RISERVATO (item.14314.name)
- RESERVED SPRITE → SPRITE RISERVATO (item.14315.name)
- RESERVED SPRITE → SPRITE RISERVATO (item.14316.name)
- RESERVED SPRITE → SPRITE RISERVATO (item.14317.name)
- RESERVED SPRITE → SPRITE RISERVATO (item.14318.name)
- RESERVED SPRITE → SPRITE RISERVATO (item.14319.name)
- ornamented wall → muro ornato (item.1432.name)
- RESERVED SPRITE → SPRITE RISERVATO (item.14320.name)
- RESERVED SPRITE → SPRITE RISERVATO (item.14321.name)
- RESERVED SPRITE → SPRITE RISERVATO (item.14322.name)
- RESERVED SPRITE → SPRITE RISERVATO (item.14323.name)
- RESERVED SPRITE → SPRITE RISERVATO (item.14324.name)

### 🚫 Raporty strażnika jakości
- Raporty strażnika jakości: **4963**  
- Raporty blokad: **0**  
- Widoczne raporty 'nie mogę przetłumaczyć': **4816**

### 🧱 Blockers snapshot (strict)
| State | Candidates | Missing files | Missing keys | Reason | Timestamp |
|-------|------------|---------------|--------------|--------|-----------|
| 🟠 active | 96 | 0 | 0 | - | 2026-04-29T13:28:55.977010Z |

### 🌐 Globalne info wszystkich języków
- **Pokrycie globalne (nominal):** **18.05%** (699,480/3,875,872)
- **Pokrycie globalne (real):** **10.97%** (425,213/3,875,872)
- **Kopie EN (łącznie):** **410,354**
- **Placeholdery [EN]/puste (łącznie):** **2,186,183**
...
| 🕒 Evidence window | 24h (span=42.61h) |
...
| Trend 24h `identical_to_en` + `word_salad` | ✅ PASS | window=42.61h; client.json: id=+0.00pp, ws=+0, otclient_modules.json: id=-1.38pp, ws=+0 |
...
| Metryka | Wartość |
|---------|---------|
| Okno | **1.0h** (2026-04-29T12:29:06.010032Z → 2026-04-29T13:29:06.010032Z) |
...
| Przepustowość (kluczy/h) | **602.0 kluczy/h** |
| Podejrzane tłumaczenia | **1010** |
| Najgorsze cele (strażnik) | cs/items.json (gf=18), ro/items.json (gf=12), ru/items.json (gf=3), it/items.json (gf=2), de/otclient_modules.json (gf=2) |
...
> Świeżość: 4min temu | Źródło: `quality_audit_latest.json` | Ostatnia aktualizacja: 2026-04-29T13:24:38.652087Z

- **Ostatni audyt:** OK | 0 issue(s) / 100 entries | 2026-04-29T13:24:38.652087Z
...
- **Języki o najsłabszej jakości:** es(60.1, issues=748988), ro(60.1, issues=443499), de(60.1, issues=126083), cs(60.1, issues=116754), fr(60.1, issues=60621)
...
⚠️ **ES**: jakość 60%, 748988 problemów
⚠️ **PT**: jakość 91%, 57675 problemów
⚠️ **FR**: jakość 60%, 60621 problemów
⚠️ **IT**: jakość 75%, 178491 problemów
⚠️ **RU**: jakość 65%, 373247 problemów
...
|-----------|--------|-----------|
| Worker | 🟢 DZIAŁA | Cykl #18,464 |
| Heartbeat | 227s temu | 2026-04-29T13:25:27Z |
...
- **Ostatnia aktualizacja:** 2026-04-29T13:28:10.712166Z
...
- 2026-04-29 13:28:57 | AUTO_TRANSLATE:heartbeat_tick | es | ok | items.json
- 2026-04-29 13:28:56 | AUTO_TRANSLATE:auto_start | es | ok | items.json
- 2026-04-29 13:26:58 | AUTO_TRANSLATE:heartbeat_tick | it | ok | items.json
- 2026-04-29 13:25:27 | AUTO_TRANSLATE:heartbeat_tick | it | ok | items.json
- 2026-04-29 13:25:26 | AUTO_TRANSLATE:auto_start | it | ok | items.json
- 2026-04-29 13:24:37 | AUTO_TRANSLATE:auto_done | ro | ok | items.json
...
|-----|---------|--------|--------|
| Tłumaczeń netto | **148,340** | — | 📊 |
| Adaptacyjna paczka | batch=50, gf_rate=6.53%, reason=stable_fail_rate=6.5%, filter=exclude_repair | odrzucenia <5% → zwiększ | 📊 |
...
> **[HISTORY]** 🟢 AKTYWNY  
> Świeżość: teraz | Źródło: `daily/*.json / ops.jsonl` | Ostatnia aktualizacja: 2026-04-29 13:29:05
...
| Metryka | Wartość |
|---------|---------|
| ✅ Przetłumaczono | **11,055** kluczy |
| ⏭️ Pominięte | 0 |
| 🔁 Cykli | 216 |
| ❌ Błędów | 486 |
| 🌍 Aktywnych języków | 7 |
| 🏆 TOP 5 | **CS** (1,894), **RU** (1,779), **TR** (1,712), **IT** (1,603), **DE** (1,575) |
| 📊 Porównanie | ↑ 34.4% vs wczoraj (8,228) |

## 📆 Ten tydzień (7 dni)

> Suma tygodnia: **30,035** kluczy
...
| 🔄 Cykli wykonanych | **#18,464** | continuous mode |
...
| System | Status | Info |
|--------|--------|------|
| Worker v1.1 | 🟢 DZIAŁA | Cykl #18,464 |
...
📅 Auto-updated by Worker v1.1 | Last: 2026-04-29 13:29:05  
