	...
	if check >= GazVariables.MaxSummons then
		...
		if check < GazVariables.MinionsNow then
			...
			for i = 1, (GazVariables.MinionsNow - check) do
				...
				creature:sayLocalized("scripts.gaz'haragoth_summon.say_2", TALKTYPE_MONSTER_SAY)
				if monster then
			...
			if math.random(0, 100) < 25 then
				...
				creature:sayLocalized("scripts.gaz'haragoth_summon.say_1", TALKTYPE_MONSTER_SAY)
				if monster then
					monster:setSummon(sum)
				end
				creature:getPosition():sendMagicEffect(CONST_ME_SOUND_RED)
