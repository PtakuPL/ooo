...
void PlayerWheel::saveSlotPointsOnPressSaveButton(NetworkMessage &msg) {
	...
	for (auto slot : magic_enum::enum_values<WheelSlots_t>()) {
		...
		if (slotPoints > maxPointsPerSlot) {
			m_player.sendLocalizedTextMessage(MESSAGE_TRADE, "cpp.wheel.error_contact_admin");
	...
	if (!sortedTableRetry.empty()) {
		m_player.sendLocalizedTextMessage(MESSAGE_TRADE, "cpp.wheel.error_relog");
...
void PlayerWheel::checkGiftOfLife() {
	...
	m_player.sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, "cpp.wheel.gift_of_life");
