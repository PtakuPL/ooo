...
function set_light.onSay(player, words, param)
	...
	local color = split[1]
	if color == nil then
		player:sendLocalizedTextMessage(MESSAGE_FAILURE, "talkaction.gm.set_light.msg_specify_color")
		return true
	end
	...
	if tonumber(color) and tonumber(color) <= 1500 and intensity >= 0 and intensity < 33 then
		...
		player:sendLocalizedTextMessage(MESSAGE_FAILURE, "talkaction.gm.set_light.msg_usage", {1500})
	end
