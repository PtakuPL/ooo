...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "addon") or MsgContains(message, "outfit") or MsgContains(message, "hat") then
		...
		if addonProgress < 1 then
			npcHandler:sayLocalized("npc.hanna.pretty_isnt_it_1", npc, creature)
			npcHandler:setTopic(playerId, 1)
		elseif addonProgress == 1 then
			npcHandler:sayLocalized("npc.hanna.oh_youre_back_2", npc, creature)
			npcHandler:setTopic(playerId, 2)
		elseif addonProgress == 2 then
			npcHandler:sayLocalized("npc.hanna.pretty_hat_isnt_3", npc, creature)
		end
		return true
	end
	...
	if npcHandler:getTopic(playerId) == 1 then
		if MsgContains(message, "yes") then
			...
			npcHandler:sayLocalized("npc.hanna.okay_here_we_4", npc, creature)
		else
			npcHandler:sayLocalized("npc.hanna.aw_i_guess_5", npc, creature)
		end
		npcHandler:setTopic(playerId, 0)
	elseif npcHandler:getTopic(playerId) == 2 then
		if MsgContains(message, "yes") then
			if player:getItemCount(3374) < 1 then
				npcHandler:sayLocalized("npc.hanna.sorry_but_i_6", npc, creature)
			elseif player:getItemCount(5890) < 100 then
				npcHandler:sayLocalized("npc.hanna.sorry_but_you_7", npc, creature)
			elseif player:getItemCount(5902) < 50 then
				npcHandler:sayLocalized("npc.hanna.sorry_but_you_8", npc, creature)
			else
				npcHandler:sayLocalized("npc.hanna.great_job_that_9", npc, creature)
			...
			npcHandler:sayLocalized("npc.hanna.maybe_another_time_10", npc, creature)
		end
		npcHandler:setTopic(playerId, 0)
	end
