...
function addaddon.onSay(player, words, param)
	...
	if #split < 3 then
		player:sendLocalizedMessage(MESSAGE_FAILURE, "talkactions.god.add_addon.usage")
		return true
	end
	...
	if not addonValue or addonValue < 0 or addonValue > 3 then
		player:sendLocalizedMessage(MESSAGE_FAILURE, "talkactions.god.add_addon.invalid_value")
		return true
	end
	...
	if addonParam == "all" then
		...
		if not looktype then
			player:sendLocalizedMessage(MESSAGE_FAILURE, "talkactions.god.add_addon.invalid_looktype")
			return true
		end
