...
> Worker tłumaczy **53** języków. Klucze EN: **74,536**. Pokrycie globalne: **nominal 18.06% / real 10.98%**. Tempo: **705.0 kluczy/h**. Tłumaczeń netto: **149,036**.

## 🧭 META

> **[META]** 🟢 AKTYWNY  
> Świeżość: teraz | Źródło: `update_github_status()` | Ostatnia aktualizacja: 2026-04-29 14:35:06

> **Aktualizacja:** 2026-04-29 14:35:06 UTC
> **Worker:** v1.1 Simple | **Guardian:** v2.0 | **Języki:** 53 | **Klucze EN:** 74536
> **Publikacja:** git-enabled
> **LIVE:** Cykl #18,545 | Status: 🟢 DZIAŁA | Faza: AUTO_TRANSLATE | Etap: heartbeat_tick | Kategoria: es | Plik: server.json | Heartbeat: 2026-04-29T14:32:33Z
> **Okno godzinowe:** okno=1.0h | cykli=77 | pominięte=0.0% | odrzucone=6.0% | przepustowość=705.0/h  
> **Tłumaczeń netto:** 149,036

### 🧩 Status sekcji
| Sekcja | Stan | Świeżość | Powód | Źródło | Ostatnia aktualizacja |
|--------|------|----------|-------|--------|-----------------------|
| META | 🟢 AKTYWNY | teraz | - | `update_github_status()` | 2026-04-29 14:35:06 |
| LIVE | 🟢 AKTYWNY | 2min temu | - | `activity.json / worker_state.json` | 2026-04-29T14:32:33Z |
| PRE_MIGRATION | 🔒 NIEAKTYWNY | 1min temu | worker w trybie AUTO_TRANSLATE | `i18n/en/*.json (LIVE) + i18n_file_status.json + i18n_processed_files.txt` | 2026-04-29 14:35:06 |
| TRANSLATION | 🟢 AKTYWNY | 32s temu | - | `translation_guard_latest.json / translation_recent_latest.json` | 2026-04-29T14:34:40.406143Z |
| QUALITY | 🔒 NIEAKTYWNY | 2min temu | worker w trybie AUTO_TRANSLATE | `quality_audit_latest.json` | 2026-04-29T14:32:32.599822Z |
| HISTORY | 🟢 AKTYWNY | teraz | - | `daily/*.json / ops.jsonl` | 2026-04-29 14:35:06 |
| DOCTOR | 🟠 AKTYWNY | 0s temu | 1 alarmów | `doctor_alerts_latest.json` | 2026-04-29T14:35:13.182121Z |
...
> **[LIVE]** 🟢 AKTYWNY  
> Świeżość: 2min temu | Źródło: `activity.json / worker_state.json` | Ostatnia aktualizacja: 2026-04-29T14:32:33Z
...
| 🔄 **Ostatni cykl** | 97 kluczy, 9 odrzuconych, 108.0s, tryb: AUTO_TRANSLATE, cel: cs/items.json |
| ❤️ **Heartbeat** | 2026-04-29T14:32:33Z |
...
| ⏳ Oldest pending (visible tail) | 17076 s |
...
| 🌍 Recent provider mix | TM/simple(369), Free Google Translate(146), canonical_name_semantic_autofix(17), OpenAI model pool(4), term... |
| 🔁 Latest fallback | TM/simple → fallback to next provider validate fail [validate_word_salad] server.json |
| 📉 Fallback breakdown | fallback to google translate high(23), fallback to next provider validate fail(5), fallback chain exhausted... |
...
| ❤️ Pool health | `critical` (writes=539, fallbacks=35) |
...
|-------|--------|----------|-------|-----------|--------|
| `HEARTBEAT_STALE` | OK | OK | `97` | `300` | Heartbeat świeży (97s, src=worker_runtime_heartbeat) Runtime[status=healthy, reason=heartbeat_fresh, phase=... |
| `NO_WRITE` | OK | OK | `43` | `900` | Recent writes wyglądają zdrowo. |
| `LEASE_CONFLICT` | OK | OK | `0` | `0` | Brak konfliktów lease/lock w oknie 1800s. |
| `QUEUE_STARVATION` | ALERT | WARNING | `{'pending': 398, 'oldest_s': 17076}` | `{'pending_min': 200, 'oldest_s': 3600}` | Duży pending i stara kolejka sugerują głodzenie queue. |
...
> Świeżość: 1min temu | Źródło: `i18n/en/*.json (LIVE) + i18n_file_status.json + i18n_processed_files.txt` | Ostatnia aktualizacja: 2026-04-29 14:35:06
...
> **[TRANSLATION]** 🟢 AKTYWNY  
> Świeżość: 32s temu | Źródło: `translation_guard_latest.json / translation_recent_latest.json` | Ostatnia aktualizacja: 2026-04-29T14:34:40.406143Z
...
- fossilised spine → zkamenělá páteř (item.14981.name)
- fossilised spine → zkamenělá páteř (item.14982.name)
- fossilised spine → zkamenělá páteř (item.14983.name)
- fossilised spine → zkamenělá páteř (item.14984.name)
- fossilised spine → zkamenělá páteř (item.14985.name)
- fossilised spine → zkamenělá páteř (item.14986.name)
- fossilised spine parts → zkamenělé části páteře (item.14987.name)
- fossilised spine parts → zkamenělé části páteře (item.14988.name)
- fossilised spine parts → zkamenělé části páteře (item.14989.name)
- fossilised spine parts → zkamenělé části páteře (item.14990.name)
- fossilised spine parts → zkamenělé části páteře (item.14991.name)
- fossilised spine parts → zkamenělé části páteře (item.14992.name)
- fossilised spine parts → zkamenělé části páteře (item.14993.name)
- fossilised spine parts → zkamenělé části páteře (item.14994.name)
- fossilised spine parts → zkamenělé části páteře (item.14995.name)
- fossilised spine parts → zkamenělé části páteře (item.14996.name)
- fossilised spine parts → zkamenělé části páteře (item.14997.name)
- fossilised spine parts → zkamenělé části páteře (item.14998.name)
- fossilised spine parts → zkamenělé části páteře (item.14999.name)
- ice shrine → ledová svatyně (item.150.name)

### 🚫 Raporty strażnika jakości
- Raporty strażnika jakości: **4985**  
- Raporty blokad: **0**  
- Widoczne raporty 'nie mogę przetłumaczyć': **4838**

### 🧱 Blockers snapshot (strict)
| State | Candidates | Missing files | Missing keys | Reason | Timestamp |
|-------|------------|---------------|--------------|--------|-----------|
| 🟠 active | 96 | 0 | 0 | - | 2026-04-29T14:32:51.825379Z |

### 🌐 Globalne info wszystkich języków
- **Pokrycie globalne (nominal):** **18.06%** (699,801/3,875,872)
- **Pokrycie globalne (real):** **10.98%** (425,562/3,875,872)
- **Kopie EN (łącznie):** **410,326**
- **Placeholdery [EN]/puste (łącznie):** **2,185,862**
...
| Metryka | Wartość |
|---------|---------|
| Okno | **1.0h** (2026-04-29T13:35:07.146700Z → 2026-04-29T14:35:07.146700Z) |
...
| Przepustowość (kluczy/h) | **705.0 kluczy/h** |
| Podejrzane tłumaczenia | **1125** |
| Najgorsze cele (strażnik) | cs/items.json (gf=27), ro/items.json (gf=8), de/npc.json (gf=3), ru/items.json (gf=3), it/items.json (gf=2) |
...
> Świeżość: 2min temu | Źródło: `quality_audit_latest.json` | Ostatnia aktualizacja: 2026-04-29T14:32:32.599822Z
...
|-----------|--------|-----------|
| Worker | 🟢 DZIAŁA | Cykl #18,545 |
| Heartbeat | 160s temu | 2026-04-29T14:32:33Z |
...
- **Ostatnia aktualizacja:** 2026-04-29T14:34:55.866963Z
...
- 2026-04-29 14:34:56 | AUTO_TRANSLATE:heartbeat_tick | es | ok | server.json
- 2026-04-29 14:34:40 | AUTO_TRANSLATE:auto_done | cs | ok | items.json
- 2026-04-29 14:34:23 | AUTO_TRANSLATE:heartbeat_tick | cs | ok | items.json
- 2026-04-29 14:32:52 | AUTO_TRANSLATE:heartbeat_tick | cs | ok | items.json
- 2026-04-29 14:32:52 | AUTO_TRANSLATE:auto_start | cs | ok | items.json
...
> **[HISTORY]** 🟢 AKTYWNY  
> Świeżość: teraz | Źródło: `daily/*.json / ops.jsonl` | Ostatnia aktualizacja: 2026-04-29 14:35:06
...
| Metryka | Wartość |
|---------|---------|
| ✅ Przetłumaczono | **11,823** kluczy |
| ⏭️ Pominięte | 0 |
| 🔁 Cykli | 216 |
| ❌ Błędów | 524 |
| 🌍 Aktywnych języków | 7 |
| 🏆 TOP 5 | **CS** (2,076), **RU** (1,870), **TR** (1,846), **DE** (1,710), **IT** (1,663) |
| 📊 Porównanie | ↑ 43.7% vs wczoraj (8,228) |

## 📆 Ten tydzień (7 dni)

> Suma tygodnia: **30,803** kluczy
...
| 🔄 Cykli wykonanych | **#18,545** | continuous mode |
...
| System | Status | Info |
|--------|--------|------|
| Worker v1.1 | 🟢 DZIAŁA | Cykl #18,545 |
...
📅 Auto-updated by Worker v1.1 | Last: 2026-04-29 14:35:06  
