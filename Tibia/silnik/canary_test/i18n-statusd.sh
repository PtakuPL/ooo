...
aggregate_telemetry() {
    python3 - "$WORK_DIR" "$STATUS_DIR" "$STATUSD_REPORT_FILE" <<'PYAGG'
import json, sys, os
...
run_status_doctor() {
    python3 - "$WORK_DIR" "$STATUS_DIR" "$STATUSD_DOCTOR_FILE" <<'PYDOCTOR'
import json, sys, os
from datetime import datetime, timezone, timedelta
...
reason_rank = {"guardian_stuck": 3, "doctor_critical": 2, "no_progress": 1}
...
generate_daily_report() {
    python3 - "$STATUS_DIR" "$STATUSD_DAILY_REPORT_JSON" "$STATUSD_DAILY_REPORT_MD" <<'PYDAILY'
