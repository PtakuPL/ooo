...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "football") then
		npcHandler:sayLocalized("npc.shiantis.do_you_want_1", npc, creature)
		npcHandler:setTopic(playerId, 1)
	elseif MsgContains(message, "yes") then
		if npcHandler:getTopic(playerId) == 1 then
			local player = Player(creature)
			if player:getMoney() + player:getBankBalance() >= 111 then
				npcHandler:sayLocalized("npc.shiantis.here_it_is_2", npc, creature)
				player:addItem(2990, 1)
				player:removeMoneyBank(111)
			else
				npcHandler:sayLocalized("npc.shiantis.you_dont_have_3", npc, creature)
			end
