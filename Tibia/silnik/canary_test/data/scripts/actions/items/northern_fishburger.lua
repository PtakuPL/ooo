...
function northernFishburger.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if player:hasExhaustion("special-foods-cooldown") then
		player:sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, "scripts.northern_fishburger.msg_1")
		return true
	end

	player:addCondition(fishingCondition)
	player:sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, "scripts.northern_fishburger.msg_2")
