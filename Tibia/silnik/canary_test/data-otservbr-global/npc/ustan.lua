...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "cough syrup") then
		NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.ustan.say_1")
	elseif MsgContains(message, "addon") then
		if player:getStorageValue(Storage.Quest.U7_8.DruidOutfits.DruidBodyAddon) < 1 then
			NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.ustan.say_2")
	...
	elseif MsgContains(message, "paws") or MsgContains(message, "bear paws") then
		if player:getStorageValue(Storage.Quest.U7_8.DruidOutfits.DruidBodyAddon) == 1 then
			NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.ustan.say_3")
			npcHandler:setTopic(playerId, 1)
		end
	elseif MsgContains(message, "yes") then
		if npcHandler:getTopic(playerId) == 1 then
			if player:getItemCount(5896) >= 50 and player:getItemCount(5897) >= 50 then
				NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.ustan.say_4")
...
NPC_LIB.i18n.setLocalizedMessage(npcHandler, MESSAGE_GREET, "npc.ustan.greet_msg_1")
