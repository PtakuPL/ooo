...
Aktualny stan — podsumowanie (2025-11-30)
• Completed / Naprawione:
	- BS::thread_pool, g_app (Console vs Graphical), preprocesory i guardy platformowe — kompilacja rusza dalej.
	- TTFFont, TextShaper, LocaleShaping — dodane pliki, włączone do projektu, podstawowy shaping + raster działa.
	- stdext::string UTF helpers (utf8 <-> utf16, latin1) uzupełnione; format -> fmt::format wg wytycznych.
	- BitmapFont TTF branch: konwersja UTF-8 -> UTF-32 i delegacja do `m_ttf` dla pomiarów/rysowania.
	- `NetworkMessage` i transfer prostych UTF-8 ciągów: zachowanie bajtów potwierdzone jednostkowo.

• In-progress / częściowo wykonane:
	- UI (UITextEdit/WidgetText): TTF rysowany w fast-path, caret / selection i handle TTF (wstępne poprawki przygotowane, wymagane dopięcia).
	- TTFFont atlasy/push do DrawPool: obecnie działa upload całego atlasu; planujemy region update dla wydajności.
	- DB charset → `utf8mb4`: w serwerze i migration SQL zmiany już częściowo wprowadzone. Konwersja live danych (ALTER TABLE ... CONVERT TO CHARACTER SET utf8mb4) wymaga zaplanowania i backupu.

• Critical CI Issue (Sonar / GH Actions) — opis i tryb natychmiastowy:
	- Symptom: build fails with a `vcpkg` error in run-vcpkg action: "libobfuscate does not exist" and the run-vcpkg action exits with error code 1.
	- Root causes likely: `vcpkg.json` references `libobfuscate` which is not an upstream port name in the selected vcpkg baseline; `builtin-baseline` may be out of sync with vcpkg upstream; `vcpkg` commit referenced in workflow may not contain this port.
	- Immediate mitigation steps (CI):
		1) Fallback: In GH workflows, set a safe `vcpkgGitCommitId` fallback (this is already in Windows build and has been added to `build-ubuntu.yml` and `analysis-sonarcloud.yml`), or ensure the extracted commit from `vcpkg.json` is a valid 40-char hash.
		2) Run a `vcpkg search libobfuscate` locally/CI to validate port name; if not found, fix `vcpkg.json` entry to correct upstream port name or add overlay port to repository `vcpkg/ports` folder.
		3) If package is a local/3rd-party overlay, include `vcpkg` overlay or port in the repository or add it to `vcpkg/ports` and configure the run-vcpkg action to use it.
		4) Add a CI preflight: `vcpkg install --manifest` step that prints `vcpkg search` and list of required ports — fails early with a friendly log. (This preflight step was added to the SonarCloud and Ubuntu workflows.)
	- Sonar note: `sonar.sourceEncoding=UTF-8` is set; continuing failures here generally stem from compile_commands generation path mismatch or `vcpkg` partitioning causing `vcpkg` install to fail.

• Runtime/Localization / UX notes:
	- Player names: the server and client must support arbitrary Unicode characters in player names (diacritics, CJK, emoji). Ensure server DB columns & tables use `utf8mb4` and `COLLATE utf8mb4_unicode_ci` (or desired collation).
	- Translation packs: client downloads translation package only after the player chooses language in the launcher/install flow — ensure the UI and local file handling allows user names to be non-ASCII prior to that download (i.e., not relying on downloaded translation packages for username normalization).

• Quick checklist to finish D (update plan): what still must be done (high-level, actionable):
	- CI: Fix `vcpkg` manifest / baseline / overlay or port name for `libobfuscate` to stop GH actions from failing.
	- UI: Finish `UITextEdit` TTF fast path changes in `uitextedit.cpp`, avoid early return on `getTexture`, render TTF via `m_font->drawText`, and adapt selection rendering logic (filled rect only for selection background when using TTF). Add unit tests.
	- TTFFont: Implement per-atlas batching (group quads by atlas) and sub-region upload (uploadSubImage) (optional performance optimization but recommended).
	- Server: Ensure DB charsets & collations to `utf8mb4`, plus migration plan for production DBs (ALTER TABLE/CONVERT). Add tests for player names (unicode) on server and plugin messages.
	- Tests/QA: Add unit & integration tests that exercise: player name with combined characters, UI text rendering in TTF for different scripts (PL, RU, AR, CJK), and a CI job that runs the test suite in headless or smoke mode.
	- Sonar/compile: Ensure compile_commands.json generated at `build/` path consistently by workflow; ensure `sonar.cfamily.compile-commands` points to `build/compile_commands.json` (it does in `analysis-sonarcloud.yml`).

	Wyraźne kroki (dokładne, co zmienić i jak przetestować):

	1) CI `vcpkg` / `libobfuscate` (BLOCKER dla Sonar / buildów nowego kodu):
		- Sprawdź czy `libobfuscate` to faktyczny port w vcpkg: uruchom lokalnie (w katalogu repo). NOTE: in this repository we removed `libobfuscate` from `vcpkg.json` and rely on `src/framework/obfuscate.h` stub; if you need the real package for some builds, add an overlay port or correct port name.
			```bash
			./vcpkg/vcpkg search libobfuscate || vcpkg search libobfuscate
			```
		- Jeżeli port nie istnieje upstream:
			- Usuń `libobfuscate` z `vcpkg.json` lub popraw nazwę na poprawny port (jeśli nazwę pomylono), OR
			- Dodaj lokalny overlay port w `vcpkg/ports/libobfuscate/` i commituj w repo (dzięki temu run-vcpkg użyje manifestu).
		- Alternatywnie, ustaw `builtin-baseline` na wersję vcpkg, która zawiera ten port OR wybierz inny dostępny pakiet.
		- W Workflows (`build-ubuntu.yml`, `analysis-sonarcloud.yml`) dodaj krok preflight:
			```bash
			echo "DEBUG: Listing vcpkg ports..."; vcpkg search || true
			vcpkg install --manifest || (echo "vcpkg manifest install failed"; exit 1)
			```
		- Dodatkowy krok: wypisz diagnostykę `vcpkg` commit id i użyty baseline — jeżeli jest nieprawidłowy, wstaw fallback commit (or use the fallback we added) or the workflow should be modified accordingly.

	2) UI — `UITextEdit`, `UIWidgetText` (ship small, safe patches first):
		- Replace early-return on null texture: in `uitextedit.cpp`, change `if (!texture) return;` to `if (!m_font->isTTF() && !texture) return;`.
		- In `drawSelf` draw path: when `m_font->isTTF()` call `m_font->drawText(m_drawText, m_drawArea, m_color, m_textAlign);`.
		- For selection: for TTF fonts, do not call `addTexturedRect` for selection — use `g_drawPool.addFilledRect` for background and modify caret drawing accordingly.
		- `update()` in `uitextedit.cpp`: wrap `calculateGlyphsPositions` with `if (!m_font->isTTF())`.
		- Add unit tests verifying `UITextEdit` displays PL/AR/RU/CJK strings and selection / caret still behave.

	3) TTFFont / BitmapFont glue (performance and correctness):
		- `TTFFont::cacheGlyph`: instead of re-uploading full atlas, call `uploadSubImage` / region update where possible.
		- `TTFFont::drawText`: group quads per `TexturePtr` (atlas) and write to `g_drawPool` per-atlas to minimize draw calls:
			- Maintain `std::unordered_map<TexturePtr, CoordsBufferPtr> atlasBuckets` and fill with quads; after shaping, call draws per atlas.
		- `BitmapFont::drawText` should not call `g_drawPool` per glyph nor expect a single `m_texture` (for TTF), as TTF uses many atlas textures.

	4) Server: Player names and DB migration concerns:
		- Confirm all DB schema & migrations are set to `DEFAULT CHARACTER SET utf8mb4` and `COLLATE utf8mb4_unicode_ci` for columns containing player names and textual fields (accounts, players.info, messages, etc.).
		- Add migration SQL (careful with production):
			```sql
			ALTER TABLE accounts CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
			ALTER TABLE players MODIFY name VARCHAR(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
			```
		- Run tests verifying that player names with diacritics/CJK/emoji are written and read correctly.
		- Ensure server sets `mysql_set_character_set(handle, "utf8mb4")` on DB connections.

	5) Tests & QA:
		- Add unit tests for: server name insert/readback with diacritics; `NetworkMessage` send/receive of multi-codepoint strings; client `TTFFont::measureTextWidth`+`drawText` per script.
		- Add CI job to run small smoke rendering: `otclient` should attempt to draw a set of strings using default TTF fonts and log errors.

	6) SonarCloud & compile_commands.json note:
		- Confirm the `analysis-sonarcloud.yml` sets `-DCMAKE_EXPORT_COMPILE_COMMANDS=ON` and passes the exact `build/compile_commands.json` to the scanner (it does). If using different build path in other jobs, adapt the sonar `compile-commands` variable accordingly.

	7) Documentation & release notes
		- Add a short `README-UTF8.md` describing: DB migration steps, how to add new fonts for fallback, recommended vcpkg baseline, and how to run the CI preflight checks.

	Note: Where possible, prefer small, safe, incremental PRs (e.g., `uitextedit` patch + smoke tests, then `TTFFont` batching + tests, then CI vcpkg manifest fix). This keeps CI green and makes debugging easier.
