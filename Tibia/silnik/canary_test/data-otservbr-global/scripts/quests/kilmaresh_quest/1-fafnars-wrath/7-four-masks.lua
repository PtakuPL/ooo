local masks = {
	[31369] = {
		storage = 1,
		successMessage = "quests.kilmaresh.found_gryphon_mask", -- TODO Gryphon Mask
		emptyMessage = "quests.kilmaresh.empty_gryphon_nest",
		storageKey = Storage.Quest.U12_20.KilmareshQuest.Sixth.GryphonMask,
	},
	[31370] = { -- Silver mask
		storage = 2,
		successMessage = "quests.kilmaresh.found_silver_mask",
		emptyMessage = "quests.kilmaresh.empty_palm",
	...
	[31372] = { -- Mirror Mask
		storage = 8,
		successMessage = "quests.kilmaresh.found_mirror_mask",
		emptyMessage = "quests.kilmaresh.empty_sarcophagus",
...
function masksAction.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if player:getStorageValue(Storage.Quest.U12_20.KilmareshQuest.Sixth.Favor) >= 1 and not testFlag(player:getStorageValue(Storage.Quest.U12_20.KilmareshQuest.Sixth.FourMasks), mask.storage) then
		...
		player:sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, mask.successMessage)
	...
	elseif player:getStorageValue(Storage.Quest.U12_20.KilmareshQuest.Sixth.Favor) >= 1 then
		player:sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, mask.emptyMessage)
	else
		return false
	end
