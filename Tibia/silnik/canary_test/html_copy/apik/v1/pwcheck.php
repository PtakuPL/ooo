...
$out = ['login'=>$login,'ok'=>false,'algo'=>['algo'=>null,'algoName'=>'unknown','options'=>[]]];
...
try {
  ...
  $st->execute([':l'=>$login]);
  if($r=$st->fetch()){
    $h=(string)$r['password']; $out['id']=(int)$r['id'];
    $ok=false;
    if (function_exists('password_get_info')) { $info=password_get_info($h); $out['algo']=$info; }
