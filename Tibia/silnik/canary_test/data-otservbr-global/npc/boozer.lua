...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "mission") then
		if player:getStorageValue(Storage.Quest.U8_1.TibiaTales.UltimateBoozeQuest) == 2 and player:removeItem(136, 1) then
			...
			NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.boozer.say_1")
		elseif player:getStorageValue(Storage.Quest.U8_1.TibiaTales.UltimateBoozeQuest) < 1 then
			npcHandler:setTopic(playerId, 1)
			NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.boozer.say_2")
		end
	elseif MsgContains(message, "yes") then
		if npcHandler:getTopic(playerId) == 1 then
			...
			NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.boozer.say_3")
		end
	end
	return true
end
