...
function onSpeak(player, type, message)
	...
	if player:getLevel() == 1 and playerGroupType == GROUP_TYPE_NORMAL then
		player:sendLocalizedTextMessage(MESSAGE_FAILURE, "channel.msg_level1_mute")
		return false
	end

	local hasExhaustion = player:kv():get("channel-help-exhaustion") or 0
	if hasExhaustion > os.time() then
		player:sendLocalizedTextMessage(MESSAGE_FAILURE, "channel.help.msg_muted")
		return false
	end
			...
			if target then
				if playerGroupType > target:getAccountType() then
					if not target:getCondition(CONDITION_CHANNELMUTEDTICKS, CONDITIONID_DEFAULT, CHANNEL_HELP) then
						...
						player:sendLocalizedTextMessage(MESSAGE_FAILURE, "channel.help.msg_already_muted")
					end
				else
					player:sendLocalizedTextMessage(MESSAGE_FAILURE, "channel.help.msg_not_authorized_mute")
				end
			else
			...
			if target then
				if playerGroupType > target:getAccountType() then
					...
					if hasExhaustionTarget > os.time() then
						...
						player:sendLocalizedTextMessage(MESSAGE_FAILURE, "channel.help.msg_not_muted")
					end
				else
					player:sendLocalizedTextMessage(MESSAGE_FAILURE, "channel.help.msg_not_authorized_unmute")
				end
			else
				player:sendCancelMessage(RETURNVALUE_PLAYERWITHTHISNAMEISNOTONLINE)
			end
