...
local function showSelectionList(data, tempValue, tempField, onSelectCallback)
    ...
    do
        ...
        button.name:setText(tr("otclient_modules.outfit.tr_9"))
        if tempValue == 0 then
            focused = 0
        end
    end
...
function create(player, outfitList, creatureMount, mountList, familiarList, wingsList, auraList, effectsList, shaderList)
    ...
    for _, appKey in ipairs(AppearanceData) do
        updateAppearanceText(appKey, tr("otclient_modules.outfit.tr_8"))
    end
...
function newPreset()
    ...
    presetWidget.title:setText(tr("otclient_modules.outfit.tr_7"))
...
function savePreset()
    ...
    --[[     if lastSelectTitle ~= "None" then
        window.presetsList[presetId].creature:getCreature():setTitle(lastSelectTitle, "noto-12", "#0000ff")
    end ]]
    -- @
end
...
function showMounts()
    ...
    local button = g_ui.createWidget("SelectionButton", window.selectionList)
    button:setId(0)
    button.name:setText(tr("otclient_modules.outfit.tr_6"))
    button.state = 0
    focused = 0
...
function showFamiliars()
    ...
    local button = g_ui.createWidget("SelectionButton", window.selectionList)
    button:setId(0)
    button.name:setText(tr("otclient_modules.outfit.tr_5"))
    focused = 0
...
function showShaders()
    ...
    do
        ...
        button.name:setText(tr("otclient_modules.outfit.tr_4"))
...
function showHealthBars()
    ...
    do
        ...
        button.outfit:hide()
        button.name:setText(tr("otclient_modules.outfit.tr_3"))
...
function showTitle()
    ...
    do
        ...
        button.name:setText(tr("otclient_modules.outfit.tr_2"))
    ...
    if ServerData.title and #ServerData.title > 0 then
        for _, titleData in ipairs(ServerData.title) do
            ...
            button.outfit:setOutfit(previewCreature:getCreature():getOutfit())
            button.outfit:getCreature():getCreature():setTitle(titleData, "noto-12", "#0000ff")
...
function onTitleSelect(list, focusedChild, unfocusedChild, reason)
    ...
    if focusedChild then
        ...
        if titleType ~= "None" then
            previewCreature:getCreature():setTitle(titleType, "noto-12", "#0000ff")
...
function updateAppearanceTexts(outfit)
    for _, appKey in ipairs(AppearanceData) do
        updateAppearanceText(appKey, tr("otclient_modules.outfit.tr_1"))
    end
