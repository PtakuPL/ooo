...
namespace {
std::string g_defaultLocaleTag = "en";

std::string buildCanonicalTag(const LocaleInfo& info)
{
  std::string tag = info.language.empty() ? std::string("en") : info.language;
...
void LocaleShaping::setDefaultLocaleTag(const std::string& tag)
{
  if (tag.empty()) {
    g_defaultLocaleTag = "en";
    return;
  }

  g_defaultLocaleTag = buildCanonicalTag(fromBCP47(tag));
}

const std::string& LocaleShaping::getDefaultLocaleTag()
{
  return g_defaultLocaleTag;
}

ShapeParams LocaleShaping::paramsFromLocaleTag(std::string_view tag)
{
  std::string canonical = tag.empty() ? g_defaultLocaleTag : std::string(tag);
  const auto info = fromBCP47(canonical);
  ShapeParams params;
  params.direction = TextDirection::AUTO;
  applyToShapeParams(info, params, true);
  return params;
}

ShapeParams LocaleShaping::paramsFromUtf8(std::string_view utf8, std::string_view fallbackTag)
{
  std::string tag = fallbackTag.empty() ? g_defaultLocaleTag : std::string(fallbackTag);
  auto fallback = fromBCP47(tag);
  auto detected = probeUtf8(utf8, fallback.language);

  if (detected.language.empty()) detected.language = fallback.language;
  if (detected.script.empty()) detected.script = fallback.script;
  if (detected.region.empty()) detected.region = fallback.region;
  if (detected.direction == TextDirection::AUTO)
    detected.direction = directionForScript(detected.script);

  ShapeParams params;
  params.direction = TextDirection::AUTO;
  applyToShapeParams(detected, params, true);
  return params;
}
