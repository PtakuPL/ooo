...
#===============================================================================
# TRYB IDLE: GENEROWANIE DOKUMENTACJI PER-NPC
#===============================================================================
generate_npc_documentation() {
    log "${CYAN}📚 IDLE: Generowanie dokumentacji NPC...${NC}"
...
NPC_JSON_DIR = "i18n/npc"
DOCS_DIR = "docs/i18n/npc"

os.makedirs(DOCS_DIR, exist_ok=True)

if not os.path.exists(NPC_JSON_DIR):
    print("⚠️ Brak katalogu i18n/npc - najpierw uruchom migrację")
    exit(0)

generated = 0
for fname in os.listdir(NPC_JSON_DIR):
    if not fname.endswith('.json'):
        continue
    
    npc_name = fname.replace('.json', '')
    json_path = os.path.join(NPC_JSON_DIR, fname)
    doc_path = os.path.join(DOCS_DIR, f"{npc_name}.md")
    
    # Pomiń jeśli dokumentacja już istnieje i jest nowsza niż JSON
    if os.path.exists(doc_path):
        json_mtime = os.path.getmtime(json_path)
        doc_mtime = os.path.getmtime(doc_path)
        if doc_mtime > json_mtime:
            continue
    ...
    for key, val in data.items():
        if isinstance(val, dict):
            for lang in val.keys():
                if lang != 'en':
                    translations_count[lang] = translations_count.get(lang, 0) + 1
    
    if translations_count:
        md_lines.append(f"| Języki z tłumaczeniami | {len(translations_count)} |")
        best_lang = max(translations_count, key=translations_count.get)
        md_lines.append(f"| Najlepiej przetłumaczony | {best_lang} ({translations_count[best_lang]}) |")
    
    md_lines.extend(["", "## Klucze i18n", ""])
    
    for key, val in list(data.items())[:20]:  # Max 20 kluczy
        en_text = val.get('en', '') if isinstance(val, dict) else str(val)
        en_preview = en_text[:60] + "..." if len(en_text) > 60 else en_text
        md_lines.append(f"- `{key}`: {en_preview}")
    
    if len(data) > 20:
        md_lines.append(f"- *... i {len(data)-20} więcej kluczy*")
    
    md_lines.extend(["", "---", f"*Plik źródłowy: `{json_path}`*"])
...
print(f"📚 Wygenerowano/zaktualizowano {generated} plików dokumentacji")
GENDOCPY
}
