...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "mission") then
		...
		elseif player:getStorageValue(Storage.Quest.U8_6.AnInterestInBotany.Questline) == 3 then
			...
			npcHandler:sayLocalized("npc.rabaz.well_fantastic_work_1", npc, creature)
		end
	elseif MsgContains(message, "yes") then
		if npcHandler:getTopic(playerId) == 1 then
			...
			npcHandler:sayLocalized("npc.rabaz.yes_yes_thats_2", npc, creature)
		...
		elseif npcHandler:getTopic(playerId) == 2 then
			if player:removeItem(11699, 1) then
				...
				npcHandler:sayLocalized("npc.rabaz.oh_you_dont_3", npc, creature)
				npcHandler:setTopic(playerId, 0)
			end
		end
	elseif categoryTable then
		local remainingCategories = npc:getRemainingShopCategories(message:lower(), itemsTable)
		npcHandler:sayLocalized("npc.rabaz.of_course_just_4" .. remainingCategories .. ".", npc, player)
