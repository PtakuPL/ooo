local configQuest = {
	["fire"] = {
		...
		msgKeys = {
			"scripts.cults_tar.fire_enter",
			"scripts.cults_tar.fire_heating",
			"scripts.cults_tar.fire_unbearable",
			"scripts.cults_tar.fire_pass",
			"scripts.cults_tar.fire_second",
			"scripts.cults_tar.fire_cooled",
		},
	},
	["acid"] = {
		itemId = 4406,
		...
		msgKeys = {
			"scripts.cults_tar.acid_enter",
			"scripts.cults_tar.acid_burning",
			"scripts.cults_tar.acid_unbearable",
			"scripts.cults_tar.acid_pass",
			"scripts.cults_tar.acid_second",
			"scripts.cults_tar.acid_cooled",
		},
	},
}
...
local function sendConditionCults(playerId, _type, fromPos, toPos, time)
	...
	time = time + 2
	if time == 30 then
		player:sendLocalizedMessage(MESSAGE_EVENT_ADVANCE, info.msgKeys[2])
	elseif time == 60 then
		player:sendLocalizedMessage(MESSAGE_EVENT_ADVANCE, info.msgKeys[2])
	elseif time >= 90 then
		...
		if stage < 3 and stage ~= 1 and stage ~= 2 then
			...
			player:sendLocalizedMessage(MESSAGE_EVENT_ADVANCE, info.msgKeys[3])
...
function passageFloor1ToFloor2(playerId, info, time)
	...
	if time == 0 and stage < 3 then
		...
		player:sendLocalizedMessage(MESSAGE_EVENT_ADVANCE, info.msgKeys[6])
		return true
	end
	if stage == 3 then
		return true
...
function tar.onStepIn(creature, item, position, fromPosition)
	...
	for index, value in pairs(configQuest) do
		if item:getId() == value.itemId and fromPosition:compare(value.beginPos) then
			player:sendLocalizedMessage(MESSAGE_EVENT_ADVANCE, value.msgKeys[1])
		...
		elseif position:compare(value.firstTile) and player:getStorageValue(value.storageBarkless) == 1 then
			...
			player:sendLocalizedMessage(MESSAGE_EVENT_ADVANCE, value.msgKeys[4])
		...
		elseif position:compare(value.secondTile) then
			if player:getStorageValue(value.storageBarkless) == 2 then
				...
				player:sendLocalizedMessage(MESSAGE_EVENT_ADVANCE, value.msgKeys[5])
			end
		end
	end
	return true
