	...
	if MsgContains(message, "exit") then
		if player:getStorageValue(Storage.Quest.U8_6.WrathOfTheEmperor.ZumtahStatus) ~= 1 then
			if npcHandler:getTopic(playerId) < 1 then
				npcHandler:sayLocalized("npc.zumtah.you_are_searching_1", npc, creature)
				npcHandler:setTopic(playerId, 1)
			elseif npcHandler:getTopic(playerId) == 3 then
				npcHandler:sayLocalized("npc.zumtah.a_what_i_2", npc, creature)
				npcHandler:setTopic(playerId, 4)
			elseif npcHandler:getTopic(playerId) == 6 then
				npcHandler:sayLocalized("npc.zumtah.haha_no_no_3", npc, creature)
				npcHandler:setTopic(playerId, 7)
			elseif npcHandler:getTopic(playerId) == 10 then
				npcHandler:sayLocalized("npc.zumtah.oh_you_mean_4", npc, creature)
				npcHandler:setTopic(playerId, 11)
			elseif npcHandler:getTopic(playerId) == 11 then
				npcHandler:sayLocalized("npc.zumtah.no_i_really_5", npc, creature)
			...
			npcHandler:sayLocalized("npc.zumtah.its_you_why_6", npc, creature)
			player:setStorageValue(Storage.Quest.U8_6.WrathOfTheEmperor.PrisonReleaseStatus, 1)
			player:addCondition(condition)
		end
	elseif MsgContains(message, "no") then
		if npcHandler:getTopic(playerId) == 1 then
			npcHandler:sayLocalized("npc.zumtah.ive_already_told_7", npc, creature)
			npcHandler:setTopic(playerId, 2)
		elseif npcHandler:getTopic(playerId) == 4 then
			npcHandler:sayLocalized("npc.zumtah.you_are_starting_8", npc, creature)
			npcHandler:setTopic(playerId, 5)
		elseif npcHandler:getTopic(playerId) == 7 then
			npcHandler:sayLocalized("npc.zumtah.but_there_is_9", npc, creature)
			npcHandler:setTopic(playerId, 8)
		end
	elseif MsgContains(message, "yes") then
		if npcHandler:getTopic(playerId) == 2 then
			npcHandler:sayLocalized("npc.zumtah.muhahaha_where_i_10", npc, creature)
			npcHandler:setTopic(playerId, 3)
		elseif npcHandler:getTopic(playerId) == 5 then
			npcHandler:sayLocalized("npc.zumtah.pesky_persistent_human_11", npc, creature)
			npcHandler:setTopic(playerId, 6)
		elseif npcHandler:getTopic(playerId) == 8 then
			npcHandler:sayLocalized("npc.zumtah.muhahaha_then_i_12", npc, creature)
			npcHandler:setTopic(playerId, 9)
		end
	elseif MsgContains(message, "278") and npcHandler:getTopic(playerId) == 9 then
		npcHandler:sayLocalized("npc.zumtah.correct_human_and_13", npc, creature)
		npcHandler:setTopic(playerId, 10)
	elseif (MsgContains(message, "164") or MsgContains(message, "89")) and npcHandler:getTopic(playerId) == 9 then
		npcHandler:sayLocalized("npc.zumtah.wrong_answer_human_14", npc, creature)
		npcHandler:setTopic(playerId, 0)
	end
	return true
end
