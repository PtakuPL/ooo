local config = {
	teleports = {
		...
		[3] = { fromPos = Position(33584, 31388, 13), toPos = Position(33584, 31391, 13), storage = Storage.Quest.U11_80.TheSecretLibrary.LiquidDeath.Questline, value = 4, effect = CONST_ME_TELEPORT, message = "quests.secret_library.ancient_passage" },
		[4] = { fromPos = Position(33560, 31395, 13), toPos = Position(33561, 31391, 13), effect = CONST_ME_TELEPORT },
	},
	defaultMessage = "quests.secret_library.not_ready_v2",
}
				...
				if tab[i].storage then
					if player:getStorageValue(tab[i].storage) >= tab[i].value then
						...
						if tab[i].message then
							player:sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, tab[i].message)
						end
						...
						player:sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, config.defaultMessage)
