...
function refill.onSay(player, words, param)
	...
	if #refilledItems == 0 then
		player:sendLocalizedMessage(MESSAGE_LOOK, "scripts.refill.msg_1")
	else
		local itemList = table.concat(refilledItems, ", ")
		player:sendLocalizedTextMessage(MESSAGE_LOOK, "talkactions.player.refill.result", {itemList, totalCost})
	end
	return true
end
