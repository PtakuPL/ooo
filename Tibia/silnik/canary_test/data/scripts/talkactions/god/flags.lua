...
function Player.talkactionHasFlag(self, param, flagType)
	...
	if not targetPlayer then
		self:sendLocalizedTextMessage(MESSAGE_FAILURE, "god.flags.player_not_found", {playerName})
		return true
	end
	...
	if not targetPlayer:hasGroupFlag(flagValue) then
		self:sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, "god.flags.player_not_have_flag", {playerName, getFlagNameByType(flagValue)})
	else
		self:sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, "god.flags.player_have_flag", {playerName, getFlagNameByType(flagValue)})
	end
...
function Player.talkactionSetFlag(self, param, flagType)
	...
	if targetPlayer:hasFlag(flagValue) then
		self:sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, "god.flags.player_already_has_flag", {playerName, getFlagNameByType(flagValue)})
		return true
	end

	targetPlayer:setGroupFlag(flagValue)
	self:sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, "god.flags.flag_set", {getFlagNameByType(flagValue), playerName})
...
function Player.talkactionRemoveFlag(self, param, flagType)
	...
	self:sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, "god.flags.flag_removed", {getFlagNameByType(flagValue), playerName})
