...
function Monster.setFiendish(self, position, player)
	if not self or not self:isForgeable() then
		player:sendLocalizedCancelMessage("libs.monster.only_allowed_fiendish")
		return false
	end
	...
	if fiendishMonster and fiendishMonster:getId() == self:getId() then
		player:sendLocalizedCancelMessage("libs.monster.already_fiendish")
		return false
	end
	...
	if Game.makeFiendishMonster(self:getId(), false) ~= 0 then
		...
		player:sendLocalizedCancelMessage("libs.monster.not_forgeable")
	end
