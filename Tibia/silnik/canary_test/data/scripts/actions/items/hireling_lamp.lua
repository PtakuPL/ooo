...
function hirelingLamp.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if not house then
		...
		player:sendLocalizedMessage(MESSAGE_FAILURE, "scripts.hireling_lamp.msg_1")
		return false
	elseif house:getDoorIdByPosition(spawnPosition) then
		player:getPosition():sendMagicEffect(CONST_ME_POFF)
		player:sendLocalizedMessage(MESSAGE_FAILURE, "scripts.hireling_lamp.msg_2")
		return false
	elseif getHirelingByPosition(spawnPosition) then
		player:getPosition():sendMagicEffect(CONST_ME_POFF)
		player:sendLocalizedMessage(MESSAGE_FAILURE, "scripts.hireling_lamp.msg_3")
		return false
	elseif house:getOwnerGuid() ~= player:getGuid() then
		player:getPosition():sendMagicEffect(CONST_ME_POFF)
		player:sendLocalizedMessage(MESSAGE_FAILURE, "scripts.hireling_lamp.msg_4")
		return false
	end

	local hireling = getHirelingById(hirelingId)
	if not hireling then
		player:sendLocalizedMessage(MESSAGE_EVENT_ADVANCE, "scripts.hireling_lamp.msg_5")
