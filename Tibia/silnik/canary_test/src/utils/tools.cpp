	...
	for (const char ch : str) {
		if (ch == '_' || std::isspace(ch) || ch == '-') {
			...
			if (capitalizeNext) {
				...
				result += (static_cast<unsigned char>(ch) < 128 ? static_cast<char>(std::tolower(static_cast<unsigned char>(ch))) : ch);
			}
		}
	}
...
std::string formatEnumName(std::string_view name) {
	...
	std::ranges::transform(result, result.begin(), [](unsigned char c) { return (c < 128 ? static_cast<unsigned char>(std::tolower(c)) : c); });
	return result;
}
...
NameEval_t validateName(const std::string &name) {
	...
	StringVector toks;
	// Accept all printable characters (including UTF-8 multi-byte letters),
	// excluding control characters (ASCII 0x00-0x1F and 0x7F).
	const std::regex regexValidChars("^[^\\x00-\\x1F\\x7F]+$");
...
std::string formatWithArticle(const std::string &value, bool withSpace) {
	...
	auto removeArticle = [](const std::string &str) -> std::string {
		...
		for (const auto &article : articles) {
			if (str.size() > article.size() && std::equal(article.begin(), article.end(), str.begin(), [](char a, char b) {
				unsigned char ua = static_cast<unsigned char>(a);
				unsigned char ub = static_cast<unsigned char>(b);
				unsigned char la = (ua < 128 ? static_cast<unsigned char>(std::tolower(ua)) : ua);
				unsigned char lb = (ub < 128 ? static_cast<unsigned char>(std::tolower(ub)) : ub);
				return la == lb;
			})) {
	...
	const unsigned char uc = static_cast<unsigned char>(modifiedValue.front());
	const char &character = static_cast<const char&>((uc < 128) ? static_cast<unsigned char>(std::tolower(uc)) : uc);
...
bool caseInsensitiveCompare(std::string_view str1, std::string_view str2, size_t length /*= std::string_view::npos*/) {
	...
	return std::equal(str1.begin(), str1.begin() + length, str2.begin(), [](char c1, char c2) {
		...
		unsigned char lc1 = (uc1 < 128 ? static_cast<unsigned char>(std::tolower(uc1)) : uc1);
		unsigned char lc2 = (uc2 < 128 ? static_cast<unsigned char>(std::tolower(uc2)) : uc2);
		return lc1 == lc2;
	});
}
