...
function ravenHerb.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	local itemName = getItemName(config.herbId)
	local backpack = player:getSlotItem(CONST_SLOT_BACKPACK)

	if not backpack or backpack:getEmptySlots(true) < 1 then
		player:sendLocalizedMessage(MESSAGE_EVENT_ADVANCE, "scripts.event_raven_herb_bush.msg_3", { item = itemName })
		return true
	end

	if (player:getFreeCapacity() / 100) < config.herbWeight then
		player:sendLocalizedMessage(MESSAGE_EVENT_ADVANCE, "scripts.event_raven_herb_bush.msg_4", { item = itemName, weight = config.herbWeight })
		return true
	end
	...
	player:sendLocalizedMessage(MESSAGE_EVENT_ADVANCE, "scripts.event_raven_herb_bush.msg_2", { item = itemName })
