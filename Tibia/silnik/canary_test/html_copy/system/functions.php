...
function template_footer(): string
{
	...
	if(admin()) {
		$footer[] = generateLink(ADMIN_URL, __('admin_panel'), true);
	}

	if(setting('core.visitors_counter')) {
		global $visitors;
		$amount = $visitors->getAmountVisitors();
		$footer[] = __($amount > 1 ? 'visitors_counter_many' : 'visitors_counter_one', ['AMOUNT' => $amount]);
	}

	if(setting('core.views_counter')) {
		global $views_counter;
		$footer[] = __('page_views', ['AMOUNT' => $views_counter]);
	}

	if(setting('core.footer_load_time')) {
		$footer[] = __('load_time_seconds', ['TIME' => round(microtime(true) - START_TIME, 4)]);
	}
	...
	// Language selector
	$footer[] = render_language_selector();
...
/**
 * Render language selector for footer
 * @return string HTML of language selector
 */
function render_language_selector(): string
{
	...
	$html = '<div class="language-selector" style="display: inline-block; margin: 0 5px;">';
	$html .= __('language') . ': ';
	$html .= '<select name="lang" onchange="window.location.href=\'?lang=\'+this.value" style="padding: 2px; font-size: 11px;">';
 ...
 * @param string $key Translation key (e.g., 'news', 'error404')
 * @param array $replacements Associative array of replacements (e.g., ['TIME' => '125'])
...
function __($key, $replacements = [])
{
	...
	if (isset($locale[$key])) {
		...
		// Handle replacements (e.g., $TIME$, $VALUE$)
