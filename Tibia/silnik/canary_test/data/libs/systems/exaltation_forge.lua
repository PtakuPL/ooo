			...
			for i = 1, #killers do
				...
				if playerKiller then
					...
					if totalDusts < limitDusts then
						...
						playerKiller:sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, "lib.exaltation_forge.msg_received", {tostring(amount), tostring(actualTotalDusts), tostring(limitDusts)})
					else
						playerKiller:sendLocalizedTextMessage(MESSAGE_EVENT_ADVANCE, "lib.exaltation_forge.msg_limit", {tostring(amount), tostring(limitDusts)})
					end
				end
			end
		else
