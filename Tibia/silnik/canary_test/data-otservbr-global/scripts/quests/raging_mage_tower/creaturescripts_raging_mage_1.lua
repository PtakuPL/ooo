...
function ragingMage1.onDeath(creature, corpse, killer, mostDamageKiller, unjustified, mostDamageUnjustified)
	...
	if killer:getStorageValue(playerDoubleMageKill) < 2 and not mostDamageKiller:hasAchievement("Mageslayer") and Game.getStorageValue(yielothaxKillStorage) < 2000 then
		creature:sayLocalized("scripts.creaturescripts_raging_mage_1.say_4", TALKTYPE_MONSTER_YELL, false, nil, Position(33143, 31527, 2))
	...
	elseif Game.getStorageValue(yielothaxKillStorage) < 2000 then
		creature:sayLocalized("scripts.creaturescripts_raging_mage_1.say_3", TALKTYPE_MONSTER_YELL, false, nil, Position(33143, 31527, 2))
		Game.createMonster("Energized Raging Mage", Position(33142, 31529, 2))
	elseif Game.getStorageValue(yielothaxKillStorage) > 1999 then
		Game.createMonster("raging mage", Position(33142, 31529, 2))
		creature:sayLocalized("scripts.creaturescripts_raging_mage_1.say_2", TALKTYPE_MONSTER_YELL, false, nil, Position(33143, 31527, 2))
		...
		if #t ~= nil then
			for i = 1, #t do
				...
				if spectator:isPlayer() and spectator:getStorageValue(673004) < 250 then
					...
					spectator:sayLocalized("scripts.creaturescripts_raging_mage_1.say_1", TALKTYPE_MONSTER_SAY, false, nil, Position(33142, 31529, 3))
				end
			end
		end
		return true
