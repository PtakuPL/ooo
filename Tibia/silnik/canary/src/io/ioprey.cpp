			...
			if (slot->bonusTimeLeft <= amount) {
				if (slot->option == PreyOption_AutomaticReroll) {
					if (player->usePreyCards(static_cast<uint16_t>(g_configManager().getNumber(PREY_BONUS_REROLL_PRICE)))) {
						...
						player->sendLocalizedTextMessage(MESSAGE_STATUS, "player.prey.bonus_type_reset");
						player->reloadPreySlot(static_cast<PreySlot_t>(slotId));
						continue;
					}

					player->sendLocalizedTextMessage(MESSAGE_STATUS, "player.prey.cards_missing_auto");
					...
					if (player->usePreyCards(static_cast<uint16_t>(g_configManager().getNumber(PREY_SELECTION_LIST_PRICE)))) {
						slot->bonusTimeLeft = static_cast<uint16_t>(g_configManager().getNumber(PREY_BONUS_TIME));
						player->sendLocalizedTextMessage(MESSAGE_STATUS, "player.prey.bonus_time_reset");
						player->reloadPreySlot(static_cast<PreySlot_t>(slotId));
						continue;
					}

					player->sendLocalizedTextMessage(MESSAGE_STATUS, "player.prey.cards_missing_lock");
				} else {
					slot->reloadMonsterGrid(player->getPreyBlackList(), player->getLevel());
					player->sendLocalizedTextMessage(MESSAGE_STATUS, "player.prey.bonus_expired");
				}
...
void IOPrey::parsePreyAction(const std::shared_ptr<Player> &player, PreySlot_t slotId, PreyAction_t action, PreyOption_t option, int8_t index, uint16_t raceId) const {
	...
	if (!slot || slot->state == PreyDataState_Locked) {
		player->sendLocalizedMessageDialog("player.prey.dialog.unlock_store");
		return;
	}
	...
	if (action == PreyAction_ListReroll) {
		if (slot->freeRerollTimeStamp > OTSYS_TIME() && !g_game().removeMoney(player, player->getPreyRerollPrice(), 0, true)) {
			player->sendLocalizedMessageDialog("player.prey.dialog.money_reroll");
			return;
		...
		if (!player->usePreyCards(static_cast<uint16_t>(g_configManager().getNumber(PREY_SELECTION_LIST_PRICE)))) {
			player->sendLocalizedMessageDialog("player.prey.dialog.cards_list");
			return;
		}
		...
		const auto mtype = g_monsters().getMonsterTypeByRaceId(raceId);
		if (slot->isOccupied()) {
			player->sendLocalizedMessageDialog("player.prey.dialog.already_active");
			return;
		} else if (!slot->canSelect() || slot->state != PreyDataState_ListSelection) {
			player->sendLocalizedMessageDialog("player.prey.dialog.action_error");
			return;
		} else if (player->getPreyWithMonster(raceId)) {
			player->sendLocalizedMessageDialog("player.prey.dialog.creature_selected");
			return;
		} else if (mtype && !mtype->info.isPreyable) {
			player->sendLocalizedMessageDialog("player.prey.dialog.creature_invalid");
			return;
		}
		...
		if (!slot->isOccupied()) {
			player->sendLocalizedMessageDialog("player.prey.dialog.no_active_monster");
			return;
		} else if (!player->usePreyCards(static_cast<uint16_t>(g_configManager().getNumber(PREY_BONUS_REROLL_PRICE)))) {
			player->sendLocalizedMessageDialog("player.prey.dialog.cards_bonus_reroll");
			return;
		}
		...
		if (option == PreyOption_AutomaticReroll && player->getPreyCards() < static_cast<uint64_t>(g_configManager().getNumber(PREY_BONUS_REROLL_PRICE))) {
			player->sendLocalizedMessageDialog("player.prey.cards_missing_auto");
			return;
			...
			player->sendLocalizedMessageDialog("player.prey.cards_missing_lock");
			return;
		}
...
void IOPrey::parseTaskHuntingAction(const std::shared_ptr<Player> &player, PreySlot_t slotId, PreyTaskAction_t action, bool upgrade, uint16_t raceId) const {
	...
	if (!slot || slot->state == PreyTaskDataState_Locked) {
		player->sendLocalizedMessageDialog("player.task.dialog.unlock_store");
		return;
	}

	if (action == PreyTaskAction_ListReroll) {
		if (slot->disabledUntilTimeStamp >= OTSYS_TIME()) {
			const auto minutes = std::to_string((slot->disabledUntilTimeStamp - OTSYS_TIME()) / 60000);
			player->sendLocalizedMessageDialog("player.task.dialog.wait_minutes", { minutes });
			return;
		} else if (slot->freeRerollTimeStamp > OTSYS_TIME() && !g_game().removeMoney(player, player->getTaskHuntingRerollPrice(), 0, true)) {
			player->sendLocalizedMessageDialog("player.task.dialog.money_reroll");
			return;
		} else if (slot->freeRerollTimeStamp <= OTSYS_TIME()) {
		...
		if (!player->usePreyCards(static_cast<uint16_t>(g_configManager().getNumber(TASK_HUNTING_BONUS_REROLL_PRICE)))) {
			player->sendLocalizedMessageDialog("player.task.dialog.cards_reward_reroll");
			return;
		}
		...
		if (slot->disabledUntilTimeStamp >= OTSYS_TIME()) {
			...
			player->sendLocalizedMessageDialog("player.task.dialog.cards_selection");
			return;
		}
		...
		if (slot->disabledUntilTimeStamp >= OTSYS_TIME()) {
			...
			player->sendLocalizedMessageDialog("player.task.dialog.action_error");
			return;
		} else if (slot->isOccupied()) {
			player->sendLocalizedMessageDialog("player.task.dialog.already_active");
			return;
		...
		if (!g_game().removeMoney(player, player->getTaskHuntingRerollPrice(), 0, true)) {
			player->sendLocalizedMessageDialog("player.task.dialog.money_cancel");
			return;
		}
		...
		if (!slot->isOccupied()) {
			player->sendLocalizedMessageDialog("player.task.dialog.empty_slot");
			return;
		}
		...
		if (const auto &option = getTaskRewardOption(slot)) {
			...
			if (slot->upgrade && slot->currentKills >= option->secondKills) {
				...
				player->sendLocalizedMessageDialog("player.task.dialog.reward_error");
				return;
			}
			...
			const std::string rewardKey = boostChange == 20 ? "player.task.dialog.reward_bonus_100" : (boostChange == 15 ? "player.task.dialog.reward_bonus_50" : "player.task.dialog.reward_bonus_none");
			player->sendLocalizedMessageDialog(rewardKey, { std::to_string(reward) });
			slot->reloadMonsterGrid(player->getTaskHuntingBlackList(), player->getLevel());
