...
npcConfig.voices = {
	interval = 15000,
	chance = 50,
	{ i18nKey = "npc.khanna.voice_1" },
	{ i18nKey = "npc.khanna.voice_2" },
}
	...
	if MsgContains(message, "first rod") or MsgContains(message, "first wand") then
		if player:isMage() then
			if player:getStorageValue(Storage.FirstMageWeapon) == -1 then
				NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.khanna.say_1", { ItemType(itemId):getName() })
				npcHandler:setTopic(playerId, 1)
			else
				NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.khanna.say_2", { ItemType(itemId):getName() })
			end
		else
			NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.khanna.say_1")
		end
	elseif MsgContains(message, "yes") then
		if npcHandler:getTopic(playerId) == 1 then
			player:addItem(itemId, 1)
			NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.khanna.say_2")
			player:setStorageValue(Storage.FirstMageWeapon, 1)
		end
		npcHandler:setTopic(playerId, 0)
	elseif MsgContains(message, "no") and npcHandler:getTopic(playerId) == 1 then
		NPC_LIB.i18n.npcSay(npcHandler, npc, creature, "npc.khanna.say_3")
		npcHandler:setTopic(playerId, 0)
	elseif categoryTable then
		local remainingCategories = npc:getRemainingShopCategories(message:lower(), itemsTable)
		NPC_LIB.i18n.npcSay(npcHandler, npc, player, "npc.khanna.say_3", { remainingCategories })
