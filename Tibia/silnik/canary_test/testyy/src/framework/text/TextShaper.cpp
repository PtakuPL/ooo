...
static std::vector<uint32_t> applyBidiReordering(const std::vector<uint32_t>& logical,
  ...
  // Reorder line (result intentionally unused — in-place reordering of visual[])
  [[maybe_unused]] auto reorderResult =
...
std::vector<ShapedGlyph> TextShaper::shape(const std::u32string& text32,
  ...
  for (unsigned int i = 0; i < glyphCount; ++i) {
    ...
    g.x = pos[i].x_offset / 64.0f;         // offset only, not absolute position
    g.y = -(pos[i].y_offset / 64.0f);       // offset only (penX in buildQuads handles accumulation)
