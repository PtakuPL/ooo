...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "yenny the gentle") then
		npcHandler:sayLocalized("npc.zoltan.ah_yenny_the_1", npc, creature)
	...
	elseif MsgContains(message, "crunor's caress") then
		...
		npcHandler:sayLocalized("npc.zoltan.a_quite_undruidic_2", npc, creature)
		npcHandler:setTopic(playerId, 0)
	elseif MsgContains(message, "proof") then
		npcHandler:sayLocalized("npc.zoltan.i_cannot_believe_3", npc, creature)
		...
		if player:getSex() == PLAYERSEX_MALE and not player:hasOutfit(130, 2) then
			if MsgContains(message, "yes") then
				if player:getItemCount(5903) == 1 then
					npcHandler:sayLocalized("npc.zoltan.i_bow_to_4", npc, creature)
					...
					npcHandler:sayLocalized("npc.zoltan.sorry_you_dont_5", npc, creature)
				end
			else
				npcHandler:sayLocalized("npc.zoltan.this_task_is_6", npc, creature)
			end
		end
	elseif MsgContains(message, "myra") then
		...
		elseif player:getStorageValue(Storage.Quest.U7_8.MageAndSummonerOutfits.AddonHatCloak) == 11 then
			npcHandler:sayLocalized("npc.zoltan.stop_bothering_me_7", npc, creature)
		else
			npcHandler:sayLocalized("npc.zoltan.what_the_hell_8", npc, creature)
		end
	end
