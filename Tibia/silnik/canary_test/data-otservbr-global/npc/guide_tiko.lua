...
local function creatureSayCallback(npc, creature, type, message)
	...
	if table.contains({ "map", "marks" }, message) then
		npcHandler:sayLocalized("npc.guide_tiko.would_you_like_1", npc, creature)
		npcHandler:setTopic(playerId, 1)
	elseif MsgContains(message, "yes") and npcHandler:getTopic(playerId) == 1 then
		npcHandler:sayLocalized("npc.guide_tiko.here_you_go_2", npc, creature)
		local mark
	...
	elseif MsgContains(message, "no") and npcHandler:getTopic(playerId) >= 1 then
		npcHandler:sayLocalized("npc.guide_tiko.well_nothing_wrong_3", npc, creature)
		npcHandler:setTopic(playerId, 0)
	end
	return true
end
