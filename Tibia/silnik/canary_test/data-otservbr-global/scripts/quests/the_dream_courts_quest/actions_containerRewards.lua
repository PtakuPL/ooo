local containers = {
	...
	[5] = {
		...
		value = 1,
		defaultItem = false,
		textKey = "scripts.dream_courts.barrel_word",
	},
	[6] = {
		uniqueid = 23107,
		...
		value = 1,
		defaultItem = false,
		textKey = "scripts.dream_courts.statue_word",
	},
	[7] = {
		uniqueid = 23108,
		...
		value = 1,
		defaultItem = false,
		textKey = "scripts.dream_courts.bed_word",
	},
	[8] = {
		uniqueid = 23109,
		...
		if iPos == k.cPosition and item:getUniqueId() == k.uniqueid then
			if player:getStorageValue(k.storage) < k.value then
				if k.defaultItem then
					...
					player:sendLocalizedMessage(MESSAGE_EVENT_ADVANCE, k.textKey)
