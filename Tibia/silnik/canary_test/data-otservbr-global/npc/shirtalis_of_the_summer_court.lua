...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "fight") then
		if player:getStorageValue(Storage.Quest.U12_00.TheDreamCourts.DreamScar.Permission) < 1 and player:getStorageValue(Storage.Quest.U12_00.TheDreamCourts.Main.TheSummerCourt) == 1 then
			npcHandler:sayLocalized("npc.shirtalis_of_the_summer_court.we_allow_able_1", npc, creature)
			npcHandler:setTopic(playerId, 2)
		end
	elseif MsgContains(message, "yes") then
		if npcHandler:getTopic(playerId) == 2 then
			npcHandler:sayLocalized("npc.shirtalis_of_the_summer_court.you_are_now_2", npc, creature)
			player:setStorageValue(Storage.Quest.U12_00.TheDreamCourts.DreamScar.Permission, 1)
			npcHandler:setTopic(playerId, 0)
		end
	elseif MsgContains(message, "no") then
		npcHandler:sayLocalized("npc.shirtalis_of_the_summer_court.as_you_wish_3", npc, creature)
		npcHandler:setTopic(playerId, 0)
	elseif MsgContains(message, "arena") then
		npcHandler:sayLocalized("npc.shirtalis_of_the_summer_court.this_place_has_4", npc, creature)
		npcHandler:setTopic(playerId, 0)
	end
